...
integration (OSC 133), richer plugin API, settings sync, signed auto-update.
...
| Linux aarch64 | `terminale-vX.Y.Z-aarch64-unknown-linux-gnu.tar.gz` |

<sub>The Windows `.msi` is Authenticode-signed and the macOS `.pkg` is Developer-ID-signed & notarized when the corresponding release secrets are configured; otherwise the binaries are published unsigned.</sub>
