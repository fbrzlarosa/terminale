...
- Production release pipeline via `cargo-dist`: `.msi` (Windows), `.pkg` (macOS), tarballs (Linux),
  Homebrew formula, `install.sh` / `install.ps1` one-liner installers, SLSA build provenance
- Custom sign-and-notarize workflow: Authenticode signing for Windows MSI (SignPath OSS or self-managed
  PFX), Developer ID + notarization for macOS PKG, all conditional on configured secrets
