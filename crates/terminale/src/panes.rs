...
use crate::{
    DividerPath, LocalDividerSpec, LocalPaneSpec, PaneDirection, PaneId, PaneNode,
    RunningState, SplitDir, TabState,
};
use std::sync::Arc;
    ...
    if let Some(zoom_id) = tab.zoomed_pane {
        if let Some(pane) = tab.panes.get(&zoom_id) {
            ...
            if let Some(rename) = &state.renaming {
                let targets_pane = matches!(rename.target, crate::RenameTarget::Pane(pid) if pid == zoom_id);
                if targets_pane {
...
pub(crate) fn pane_header_close_at(state: &RunningState, pointer_phys: (f32, f32)) -> Option<PaneId> {
...
pub(crate) fn split_node_mut_at_path<'a>(tree: &'a mut PaneNode, path: &[bool]) -> Option<&'a mut PaneNode> {
    let mut node = tree;
    for &step in path {
        match node {
...
pub(crate) fn split_focused_pane(state: &mut RunningState, direction: SplitDir, side_b: bool) {
    ...
    let inherited_cwd: Option<std::path::PathBuf> = tab.emulator.lock().current_dir().map(std::path::PathBuf::from);
    let profile: Option<terminale_config::Profile> = inherited_cwd.map(|cwd| terminale_config::Profile {
...
pub(crate) fn keyboard_resize_pane(state: &mut RunningState, direction: PaneDirection) {
    ...
    let Some(ResizeSplitResult { path: Some(path), focused_in_a }) = result else {
        return;
    };
    ...
    let split_dir_opt = {
        ...
        for &step in &path {
            match node {
                PaneNode::Leaf(_) => { node = &tab2.tree; break; }
                PaneNode::Split { a, b, .. } => { node = if step { b } else { a }; }
            }
        }
        if let PaneNode::Split { direction, .. } = node { Some(*direction) } else { None }
    };
    ...
    let delta = match (split_dir, direction) {
        ...
        (SplitDir::Vertical, PaneDirection::Right) | (SplitDir::Vertical, PaneDirection::Left) => {
            ...
            let sign: f32 = match direction {
                PaneDirection::Right => if focused_in_a { 1.0 } else { -1.0 },
                PaneDirection::Left  => if focused_in_a { -1.0 } else { 1.0 },
                _ => return,
            };
        ...
        (SplitDir::Horizontal, PaneDirection::Down) | (SplitDir::Horizontal, PaneDirection::Up) => {
            ...
            let sign: f32 = match direction {
                PaneDirection::Down => if focused_in_a { 1.0 } else { -1.0 },
                PaneDirection::Up   => if focused_in_a { -1.0 } else { 1.0 },
                _ => return,
            };
            sign * cell_ratio * f32::from(step)
        }
...
pub(crate) fn find_resize_split_inner(
    ...
    match node {
        PaneNode::Leaf(id) => {
            ...
            if *id == target {
                Some(ResizeSplitResult { path: None, focused_in_a: true })
            } else {
                None
            }
        }
        PaneNode::Split { direction, a, b, .. } => {
            // Descend into `a`.
            path.push(false);
...
mod tests {
    ...
    fn rotate_forward_and_back_are_inverses() {
        ...
        rotate_panes(&mut tree);
        rotate_panes_back(&mut tree);
        assert_eq!(leaf_ids(&tree), original, "forward then back must restore original order");
    }
    ...
    fn two_pane_horizontal_layout() -> Vec<(PaneId, (f32, f32, f32, f32))> {
        vec![(1, (0.0, 0.0, 100.0, 100.0)), (2, (100.0, 0.0, 100.0, 100.0))]
    }
