...
impl ContextMenuWindow {
    ...
    fn resize_for_flyout(&mut self) {
        ...
        if let Some(work_area) = self.monitor_work_area {
            ...
            let y = menu_top_y(self.origin_screen.y, (lh * scale) as i32, work_top, work_bottom);
                ...
                menu_frame().show(&mut col_ui, |ui| {
                    ...
                    for (i, entry) in self.entries.iter().enumerate() {
                        ...
                        if ui.is_rect_visible(rect) {
                            draw_row(ui, entry, rect, resp.hovered() && entry.enabled, icon_gutter);
                        }
                            ...
                            for (ci, child) in children_owned.iter().enumerate() {
                                if child.separator_before && ci > 0 {
                                    ...
                                    ui.painter().line_segment(
                                        ...
                                        egui::Stroke::new(
                                            1.0,
                                            egui::Color32::from_rgb(48, 56, 80),
                                        ),
                                    );
                                    ui.add_space(4.0);
                                }
...
mod tests {
    ...
    fn submenu_transition_different_parent_fires_open() {
        ...
        assert!(fire, "switching to a different submenu parent must trigger resize");
    }
    ...
    fn submenu_transition_leaf_outside_flyout_closes() {
        ...
        assert_eq!(new_idx, None, "leaving the parent row must close the flyout");
        assert!(resize, "closing the flyout must resize the window back down");
    }
    ...
    fn desired_size_with_flyout_is_wider() {
        ...
        assert!(w > MENU_WIDTH, "window with flyout must be wider than MENU_WIDTH");
    }
