...
impl SettingsWindow {
    pub(super) fn section_terminal(&mut self, ui: &mut egui::Ui) {
        ...
        card(ui, |ui| {
            ...
            self.highlight_row(ui, hr.response.rect, Section::Terminal, "Alt-screen scroll step");
            sublabel(
                ui,
            ...
            self.highlight_row(ui, hr.response.rect, Section::Terminal, "Trackpad pixels per row");
            sublabel(
                ui,
        ...
        card(ui, |ui| {
            let hr = ui.horizontal(|ui| {
                ...
                let r = ui.add(
                    egui::Slider::new(
                        &mut self.config.terminal.link_hover_delay_ms,
                        0..=2000,
                    )
                    .suffix(" ms")
                    .text(""),
        ...
        card(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button("+ Add rule").clicked() {
                    self.config.terminal.hyperlink_rules.push(
                        terminale_config::HyperlinkRule::default(),
                    );
                    dirty = true;
                }
        ...
        card(ui, |ui| {
            ...
            let hr = ui.horizontal(|ui| {
                ...
                let r = ui.add(
                    egui::Slider::new(
                        &mut self.config.terminal.max_command_blocks,
                        1..=100_000,
                    )
                    .logarithmic(true)
                    .suffix(" blocks")
        ...
        if dirty {
            crate::osc_handlers::update_exit_behavior(
                self.config.terminal.exit_behavior,
            );
