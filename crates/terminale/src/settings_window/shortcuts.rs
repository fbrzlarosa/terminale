    ...
    pub(super) fn section_shortcuts(&mut self, ui: &mut egui::Ui) {
        ...
        let groups: &[(&str, &[(&str, &str, Getter, &str)])] = &[
            (
                ...
                &[
                    ...
                    ("new_ssh_tab", "New SSH tab", |s| &mut s.new_ssh_tab, ""),
                    ("last_tab", "Go to last-used tab", |s| &mut s.last_tab, ""),
                ],
            ),
            (
                "Tab index",
            ...
            (
                ...
                &[
                    ...
                    (
                        ...
                        |s: &mut terminale_config::ShortcutsConfig| &mut s.toggle_broadcast_input,
                        "",
                    ),
                ],
            ),
            ...
            for (tidx, table) in tables.iter_mut().enumerate() {
                ui.push_id(tidx, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        let r = ui.add(egui::Slider::new(&mut t, 100_u32..=30_000_u32).text("ms"));
                        if r.changed() {
                            table.timeout_ms = t;
                            dirty = true;
                        }
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if r.changed() {
                                    binding.actions = actions_str
                                        ...
                                        .filter(|s| !s.is_empty())
                                        .map(|s| {
                                            terminale_config::KeyActionSpec::Action(s.to_string())
                                        })
                                        .collect();
                                    dirty = true;
                                }
        ...
        if add_table {
            self.config
                .keybinds
                .key_tables
                .push(terminale_config::KeyTable {
                    name: String::new(),
                    leader: String::new(),
                    timeout_ms: 1500,
                    bindings: Vec::new(),
        ...
        card(ui, |ui| {
            ...
            const BUTTON_OPTIONS: &[&str] = &["Left", "Right", "Middle", "Back", "Forward"];
                ...
                ui.push_id(idx, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        egui::ComboBox::from_id_salt(format!("mb_btn_{idx}"))
                            .selected_text(BUTTON_OPTIONS.get(selected).copied().unwrap_or("Left"))
                            .width(90.0)
                            .show_ui(ui, |ui| {
                        ...
                        egui::ComboBox::from_id_salt(format!("mb_cnt_{idx}"))
                            ...
                            .show_ui(ui, |ui| {
                                for (i, label) in count_labels.iter().enumerate() {
                                    if ui.selectable_label(count_idx == i, *label).clicked() {
