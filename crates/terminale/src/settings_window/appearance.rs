    ...
    pub(super) fn section_appearance(&mut self, ui: &mut egui::Ui) {
        ...
        // ── Theme import ──
        {
            ...
            card(ui, |ui| {
                ...
                let hr = ui.horizontal(|ui| {
                    ...
                    let hint = terminale_config::paths::themes_dir()
                        .map_or_else(|| "<config dir>/themes".to_owned(), |p| p.to_string_lossy().into_owned());
                    let r = ui.add(
                ...
                self.highlight_row(ui, hr.response.rect, Section::Appearance, "Themes directory");
                sublabel(
                    ui,
                ...
                let drop_ins: Vec<String> = if let Some(dir) = self.config.appearance.effective_themes_dir() {
                ...
                if !drop_ins.is_empty() {
                    ui.add_space(6.0);
                    sublabel(ui, &format!("Drop-in themes loaded from directory ({}):", drop_ins.len()));
        ...
        // ── Background image ──
        {
            ...
            card(ui, |ui| {
                let hr = ui.horizontal(|ui| {
                    ...
                    if r.changed() {
                        self.config.appearance.background_image.path =
                            if path_edit.is_empty() { None } else { Some(path_edit) };
                        dirty = true;
                    }
                    if ui.button("\u{1f4c2}").on_hover_text("Browse…").clicked() {
                ...
                sublabel(ui, "Absolute path to an image file drawn behind the terminal text.");
            });
            ui.add_space(6.0);
        ...
        // ── Faint/dim intensity ───────────────────────────────────────────────
        {
            ...
            card(ui, |ui| {
                let hr = ui.horizontal(|ui| {
                    ...
                    let r = ui.add(
                        egui::Slider::new(
                            &mut self.config.appearance.dim_amount,
                            0.0..=1.0,
                        )
                        .fixed_decimals(2)
                        .text(""),
            ...
            card(ui, |ui| {
                let hr = ui.horizontal(|ui| {
                    ...
                    let r = ui.add(
                        egui::Slider::new(
                            &mut self.config.appearance.minimum_contrast,
                            1.0..=21.0,
                        )
                        .fixed_decimals(1)
                        .text(""),
        ...
        card(ui, |ui| {
            ...
            self.highlight_row(ui, hr.response.rect, Section::Appearance, "Pinned tab width");
            sublabel(
                ui,
        ...
        card(ui, |ui| {
            let hr = ui.horizontal(|ui| {
                let r = ui.checkbox(
                    &mut self.config.appearance.pane_tear_out,
                    "Tear out panes",
                );
                if r.changed() {
                    dirty = true;
        ...
        card(ui, |ui| {
            ...
            self.highlight_row(ui, hr.response.rect, Section::Appearance, "Tab bar position");
            sublabel(
                ui,
        ...
        card(ui, |ui| {
            ...
            for (i, rgb) in self.config.appearance.tab_group_colors.iter_mut().enumerate() {
                ui.horizontal(|ui| {
            ...
            if ui.small_button("Add colour").clicked() {
                self.config.appearance.tab_group_colors.push([0x7d, 0xa6, 0xff]);
                dirty = true;
            }
        });
        ...
        card(ui, |ui| {
            let hr = ui.horizontal(|ui| {
                ...
                let r = ui.add(
                    egui::Slider::new(
                        &mut self.config.appearance.inactive_pane_dim,
                        0.0..=0.9,
                    )
                    .fixed_decimals(2)
                    .text(""),
        ...
        card(ui, |ui| {
            let hr = ui.horizontal(|ui| {
                ...
                let r = ui.add(
                    egui::Slider::new(
                        &mut self.config.appearance.unfocused_window_dim,
                        0.0..=0.9,
                    )
                    .fixed_decimals(2)
                    .text(""),
        ...
        card(ui, |ui| {
            ...
            self.highlight_row(ui, hr.response.rect, Section::Appearance, "Use bundled icon set");
            sublabel(
                ui,
