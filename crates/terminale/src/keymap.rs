...
pub(crate) fn action_from_name(name: &str) -> Option<ShortcutAction> {
    Some(match name.to_lowercase().as_str() {
        ...
        "rotatepaneback" | "rotatepanersback" | "rotatepanesback" => ShortcutAction::RotatePanesBack,
        ...
        "openfailedcommandpicker" | "failedcommandpicker" => ShortcutAction::OpenFailedCommandPicker,
...
mod tests {
    ...
    fn action_from_name_case_insensitive() {
        assert!(matches!(action_from_name("newtab"), Some(ShortcutAction::NewTab)));
        assert!(matches!(action_from_name("NewTab"), Some(ShortcutAction::NewTab)));
        assert!(matches!(action_from_name("NEWTAB"), Some(ShortcutAction::NewTab)));
    }
    ...
    fn multi_action_sequence_order_preserved() {
        ...
        assert!(
            matches!(resolved[0], ResolvedAction::Shortcut(ShortcutAction::NewTab)),
    ...
    fn key_table_entry_actions_resolve() {
        ...
        assert_eq!(resolved.len(), 2, "two actions expected");
        assert!(
            matches!(resolved[0], ResolvedAction::Shortcut(ShortcutAction::NextTab)),
