...
use crate::{
    ParsedSsh, RunningState, SshConnectOutcome, UserEvent,
};
use std::sync::Arc;
...
pub(crate) fn ssh_host_targets_from(cfg: &terminale_config::Config) -> Vec<(String, Option<String>, u16)> {
    cfg.ssh_hosts
        .iter()
        .map(|h| {
...
pub(crate) fn snippet_names_from(
...
pub(crate) fn handle_save_prompt_click(state: &mut RunningState, hit: terminale_render::SavePromptHit) {
