...
pub(crate) fn refresh_tab_bar(state: &mut RunningState) {
    ...
    let rename_tab: Option<(usize, String)> = state.renaming.as_ref().and_then(|r| {
        match r.target {
    ...
    let rename_info: Option<(usize, crate::RenameTarget, String)> =
        state.renaming.as_ref().map(|r| (r.tab_idx, r.target, r.buffer.clone()));
    ...
    if let Some(bar) = state.renderer.tab_bar_mut() {
        ...
        for (idx, t) in state.tabs.iter().enumerate() {
            let group_accent = t
                ...
                .map(|g| g.color);
            let group_label: Option<String> = if let Some(gid) = t.group {
                let prev_group = if idx > 0 { state.tabs[idx - 1].group } else { None };
                if prev_group == Some(gid) {
                    None
                } else {
...
pub(crate) fn new_tab_inner(state: &mut RunningState, profile: Option<&Profile>) {
    ...
    let inherited_cwd: Option<std::path::PathBuf> = state
        .tabs
        .get(state.active_tab)
        .and_then(|t| t.emulator.lock().current_dir().map(std::path::PathBuf::from));
    ...
    if let Some(t) = state.tabs.last() {
        ...
        emu.set_command_blocks(
            state.command_blocks_enabled,
            state.max_command_blocks,
        );
    }
    state.renderer.set_selection(None);
    state.window.request_redraw();
...
pub(crate) fn close_tab(state: &mut RunningState, idx: usize) {
    ...
    let restore = state.tabs.get(idx).map(|tab| ClosedTab {
        ...
        icon: tab.icon.clone(),
        cwd: tab.emulator.lock().current_dir().map(std::path::PathBuf::from),
    });
...
pub(crate) fn close_exited_pane(state: &mut RunningState, pane_id: crate::PaneId) {
    ...
    let original_focus = state
        .tabs
        .get(active)
        .map_or(pane_id, |t| t.focused);
...
pub(crate) fn restart_active_tab(state: &mut RunningState) {
    ...
    let Ok(mut session) = terminale_core::Session::spawn_with_notifier(&spec, initial.0, initial.1, notifier)
    else {
