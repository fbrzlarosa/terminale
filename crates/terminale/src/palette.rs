...
use crate::{
    CommandPaletteState, PaletteItem, PaletteMode, RunningState, ShortcutAction,
};
...
pub(crate) const PALETTE_ACTIONS: &[(ShortcutAction, &str)] = &[
    ...
    (ShortcutAction::MovePaneToNewWindow, "Move Pane to New Window"),
    ...
    (ShortcutAction::CopyLastCommandOutput, "Copy Last Command Output"),
    ...
    (ShortcutAction::SnapBottomRight, "Snap: Bottom-Right Quarter"),
    ...
    (ShortcutAction::JumpToPrevFailedCommand, "Jump to Previous Failed Command"),
    (ShortcutAction::JumpToNextFailedCommand, "Jump to Next Failed Command"),
    (ShortcutAction::OpenFailedCommandPicker, "Failed Commands\u{2026}"),
    ...
    (ShortcutAction::ToggleBroadcastInput, "Toggle Broadcast Input"),
    ...
    (ShortcutAction::ImportSshHosts, "Import SSH Hosts from SSH Config"),
    (ShortcutAction::OpenCommandHistory, "Command History\u{2026}"),
    (ShortcutAction::ExportScrollback, "Export Scrollback\u{2026}"),
    (ShortcutAction::OpenClipboardHistory, "Clipboard History\u{2026}"),
...
pub(crate) fn snippet_rows(
    ...
    snippet_names
        ...
        .enumerate()
        .map(|(idx, (name, desc))| {
            (PaletteItem::InsertSnippet(idx), name.clone(), desc.clone())
        })
        .collect()
}
...
pub(crate) fn command_history_rows(
    history: &[String],
) -> Vec<(PaletteItem, String, String)> {
    history
        .iter()
...
pub(crate) fn clipboard_history_rows(
    entries: &[String],
) -> Vec<(PaletteItem, String, String)> {
    entries
        .iter()
        .map(|text| {
            ...
            let label = text.lines().collect::<Vec<_>>().join(" ").trim().to_string();
            (
...
pub(crate) fn directory_jump_rows(
    dirs: &[String],
) -> Vec<(PaletteItem, String, String)> {
    dirs.iter()
        .map(|path| {
...
pub(crate) fn plugin_command_rows(
    names: &[String],
) -> Vec<(PaletteItem, String, String)> {
    names
        .iter()
...
pub(crate) fn open_command_history(state: &mut RunningState) {
    ...
    state.command_history_cache =
        build_command_history(raw.iter().map(String::as_str), max);
...
pub(crate) fn refresh_palette(state: &mut RunningState) {
    ...
    let placeholder = match mode {
        PaletteMode::Actions => "Search commands \u{2014} type to filter, \u{2191}\u{2193} to move, Enter to run",
...
mod tests {
    ...
    fn history_filters_empty_and_whitespace_only() {
        ...
        assert_eq!(result.len(), 2, "empty/whitespace commands must be filtered");
    ...
    fn history_fuzzy_filters_correctly() {
        ...
        assert_eq!(ranked.len(), 2, "fuzzy filter must exclude non-matching entry");
    ...
    fn fuzzy_score_prefix_scores_higher_than_interior() {
        ...
        assert!(prefix > interior, "prefix match must outscore interior match");
    }
    ...
    fn clipboard_history_fuzzy_filter() {
        ...
        assert!(
            matches!(&ranked[0].0, PaletteItem::PasteClipboardEntry(t) if t == "cargo build")
        );
    }
}
