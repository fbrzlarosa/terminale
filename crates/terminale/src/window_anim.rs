...
use crate::{RunningState, QuakeAnim};
...
pub(crate) fn apply_zen_chrome(state: &mut RunningState) {
    ...
    let hide_status_bar =
        state.zen && hide.iter().any(|e| matches!(e, ZenHideElement::StatusBar));
    let hide_pane_headers =
        state.zen && hide.iter().any(|e| matches!(e, ZenHideElement::PaneHeaders));
...
pub(crate) fn apply_window_rect(
    window: &Window,
    rect: terminale_config::WindowRect,
    resize: bool,
) {
    let (x, y, w, h) = rect;
    if resize {
...
pub(crate) fn open_snap_chooser(state: &mut RunningState) {
    ...
    state.renderer.set_snap_chooser(Some(terminale_render::SnapChooserOverlay {
        hovered: None,
    }));
    state.window.request_redraw();
}
...
pub(crate) fn rects_close(a: terminale_config::WindowRect, b: terminale_config::WindowRect, tol: i32) -> bool {
    (a.0 - b.0).abs() <= tol
        && (a.1 - b.1).abs() <= tol
...
pub(crate) fn lerp_rect_full(
    ...
    (lerp_i(a.0, b.0), lerp_i(a.1, b.1), lerp_u(a.2, b.2), lerp_u(a.3, b.3))
}
...
) -> Option<(terminale_config::WindowRect, Option<terminale_config::MonitorRect>)> {
...
pub(crate) fn cursor_params_from_config(cfg: &terminale_config::Config) -> terminale_render::CursorParams {
...
pub(crate) fn gpu_options_from_config(cfg: &terminale_config::Config) -> terminale_render::GpuOptions {
...
pub(crate) fn translate_bg_fx_params(
    ...
    terminale_render::BgFxParams {
        ...
        max_emitters: cfg.max_emitters.clamp(1, terminale_render::MAX_EMITTERS as u32),
    }
}
