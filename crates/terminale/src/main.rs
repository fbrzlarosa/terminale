...
use terminale_render::{
    CellRect, LabelBadge, MenuItem, MenuOverlay, Renderer, TabBar, TabBarItem,
    TabHit, WindowCtrl,
};
...
impl std::fmt::Debug for SshConnectOutcome {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SshConnectOutcome")
            ...
            .field("result", &self.result.as_ref().map(|_| "<SshSession>").map_err(String::as_str))
            .finish()
    }
}
...
impl TerminaleApp {
            ...
            DragPayload::Pane { tab_index, pane_id } => {
                match drag.target {
                    DropTarget::Reorder(_) => {
                        ...
                        self.attach_pane(drag.origin_window, tab_index, pane_id, drag.origin_window, drag.origin_window, None);
                    }
                    DropTarget::AttachTo(target_id, _slot) => {
                        ...
                        self.attach_pane(drag.origin_window, tab_index, pane_id, target_id, drag.origin_window, None);
                    }
                    DropTarget::Detach => {
                        // Dropped outside every window — tear the pane out
                        // into a new OS window at the cursor position.
                        self.tear_out_pane(event_loop, drag.origin_window, tab_index, pane_id, drag.cursor_screen);
                    }
                }
            }
    ...
    fn attach_pane(
        ...
        let (cols, rows) = dst.renderer.pixels_to_cells(dst_size.width, dst_size.height);
    ...
    fn tear_out_pane(
        ...
        let spawn_pos = winit::dpi::PhysicalPosition::new(
            cursor_screen.0 - 60,
            cursor_screen.1 - 18,
        );
        ...
        let new_tab =
            build_single_pane_tab(pane, src_palette, terminale_term::DEFAULT_COLS, terminale_term::DEFAULT_ROWS);
        let mut new_win =
    ...
    fn new_window(&mut self, event_loop: &ActiveEventLoop, src_idx: usize) {
        ...
        // exactly overlap.
        let spawn_pos = src.window.outer_position().ok().map(|p| {
            winit::dpi::PhysicalPosition::new(p.x + 40, p.y + 40)
        });
    ...
    fn restore_workspace(
        ...
        let tab_plans: Vec<RestoreTabPlan> = saved
            ...
            .map(|saved_tab| {
                ...
                let init =
                    if let crate::workspace::RestoreStep::InitLeaf(ref leaf) = steps[0] {
                ...
                let splits: Vec<_> = steps
                    ...
                    .filter_map(|step| {
                        ...
                        {
                            let profile =
                                self.resolve_leaf_profile(&leaf.profile, &leaf.cwd);
        ...
        let saved_tab_groups: Vec<Option<u32>> =
            saved.tabs.iter().map(|t| t.group).collect();
    ...
    fn resolve_leaf_profile(
        ...
        match (base, cwd.as_deref(), self.config.window.restore_working_dirs) {
    ...
    fn tear_out_group(
        ...
        // Recompute source active tab.
        {
            ...
            src.active_tab =
                active_tab_after_detach(src.active_tab, first_removed, len_before)
        ...
        let mut new_win =
            self.build_window(event_loop, Some(shared), Some(new_pos), removed);
        new_win.active_tab = 0;
    ...
    fn attach_group(
        ...
        // Splice into destination.
        {
            ...
            let (cols, rows) = dst.renderer.pixels_to_cells(win_size.width, win_size.height);
    ...
    fn spawn_suggestion(&mut self, win_idx: usize) {
        ...
        let messages =
            terminale_ai::suggestion_messages(&context, &current_line, &shell, std::env::consts::OS);
        ...
        self.runtime.handle().spawn(async move {
            let provider =
                terminale_ai::build_provider(&provider_name, secret, ollama_url);
        ...
        match Config::load_or_init_at(Some(self.config_path.clone())) {
            Ok((new_cfg, _)) => {
                ...
                for state in &mut self.windows {
                    ...
                    state
                        .renderer
                        .set_dim_amount(cfg.appearance.dim_amount);
                    state
                        .renderer
                    ...
                    state.renderer.set_tab_pinned_width(cfg.appearance.pinned_tab_width);
                    if font_changed {
                    ...
                    state.renderer.set_background_image(translate_bg_image_params(
...

        {
            ...
            if let Some(demo) = std::env::var_os("TERMINALE_DEMO_PALETTE") {
                if demo == "explain" {
                    ...
                    demo_cfg.status_bar.position =
                        terminale_config::StatusBarPosition::Bottom;
                    ...
                    demo_cfg.status_bar.right_segments = vec![
                        terminale_config::StatusSegment::Clock {
                            format: "%H:%M".into(),
                        },
                    ];
                    if let Some(tab) = state.tabs.get(state.active_tab) {
                    ...
                    demo_cfg.background_fx.style =
                        terminale_config::BackgroundFxStyle::Matrix;
                    ...
                    state.key_tables.clone_from(&self.config.keybinds.key_tables);
                    ...
                    self.config.context_rules = vec![
                        terminale_config::ContextRule {
                    ...
                    let osc7 = format!("\x1b]7;file://localhost{}\x1b\\", demo_cwd.replace('\\', "/"));
                    if let Some(tab) = state.tabs.get(state.active_tab) {
                    ...
                    let themes_dir = self.config.appearance.effective_themes_dir()
                        .or_else(terminale_config::paths::themes_dir);
                    if let Some(dir) = themes_dir {
                        let _ = std::fs::create_dir_all(&dir);
                        let theme_toml = toml::to_string_pretty(&demo_theme)
                            .unwrap_or_default();
                    ...
                    if !self.config.appearance.themes.iter().any(|t| t.name == demo_theme.name) {
                ...
                if demo == "tabgroups" {
                    ...
                    if let Some(tab) = state.tabs.get_mut(0) { tab.group = Some(id_a); }
                    if let Some(tab) = state.tabs.get_mut(1) { tab.group = Some(id_b); }
                    if let Some(tab) = state.tabs.get_mut(2) { tab.group = Some(id_b); }
                    if let Some(tab) = state.tabs.get_mut(3) { tab.group = Some(id_a); }
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, id: WindowId, event: WindowEvent) {
            ...
            if menu.id() == id {
                ...
                if let Some(action_id) = action {
                    ...
                    if action_id >= GROUP_ASSIGN_BASE {
                        ...
                        if let Some(glyph) = crate::settings_window::ICON_PRESETS.get(idx).map(|(_, g)| *g) {
                            if let Some(state) = self.focused_window_mut() {
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                if state.snap_chooser_open {
                    let hovered =
                        state.renderer.snap_chooser_hit(position.x as f32, position.y as f32);
            ...
            WindowEvent::KeyboardInput {
                ...
                if state.quick_select.is_some()
                    && handle_quick_select_input(state, &logical_key)
                {
                    state.window.request_redraw();
                    return;
                }
                ...
                if state.pane_select.is_some()
                    && handle_pane_select_input(state, &logical_key)
                {
                    state.window.request_redraw();
                    return;
                }
                ...
                if handle_key_table_leader(state, physical_key, &logical_key, &self.config.keybinds.key_tables) {
                ...
                if let Some(bytes) =
                    translate_key(&state.modifiers, physical_key, &logical_key, text, app_cursor)
                {
                    if let Some(tab) = state.tabs.get(state.active_tab) {
        ...
        if std::mem::replace(&mut state.open_ssh_picker, false) {
            ...
            let entries: Vec<MenuEntry> = self
                ...
                .enumerate()
                .map(|(idx, h)| MenuEntry {
                    icon: Some(crate::icons::glyph(&crate::icons::WORLD, self.config.appearance.bundled_icons).into()),
        ...
        if let Some(origin) = state.open_menu_at.take() {
            ...
            let demo_ctxmenu = std::env::var_os("TERMINALE_DEMO_PALETTE")
                .is_some_and(|v| v == "ctxmenu");
        ...
        if do_move_pane_to_new_tab {
            if let Some(pane_id) = focused_pane_snap {
                self.attach_pane(win_id_snap, active_tab_snap, pane_id, win_id_snap, win_id_snap, None);
            }
        }
        ...
        if do_move_pane_to_new_window {
            if let Some(pane_id) = focused_pane_snap {
                ...
                let spawn_pos = {
                    ...
                    w.and_then(|w| w.window.outer_position().ok()).map(|p| {
                ...
                self.tear_out_pane(event_loop, win_id_snap, active_tab_snap, pane_id, (spawn_pos.x, spawn_pos.y));
            }
        }
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if self.plugins.is_some() {
            ...
            for state in &mut self.windows {
                ...
                // tab_close
                let tab_closes: Vec<usize> =
                    std::mem::take(&mut state.pending_hook_tab_close);
                ...
                // pane_focus
                let pane_focuses: Vec<u32> =
                    std::mem::take(&mut state.pending_hook_pane_focus);
                        ...
                        for (&pane_id, pane) in &tab.panes {
                            ...
                            if new_count > already_fired {
                                for block in &completed[already_fired..] {
                                    ...
                                    state.pending_hook_command_end
            ...
            if let Some(settings) = self.settings.as_mut() {
                if settings.available_fonts.is_empty() {
                    if let Some(state) = self.windows.first() {
                        settings.available_fonts =
                            state.renderer.available_monospace_families();
        ...
        if self.settings.as_ref().is_some_and(|s| s.pending_import_theme) {
            if let Some(s) = self.settings.as_mut() {
        ...
        // can't call the App method directly — it sets this flag and we act
        // on it here, where `&mut self` (TerminaleApp) is available.
        if self.settings.as_ref().is_some_and(|s| s.pending_import_ssh_hosts) {
            if let Some(s) = self.settings.as_mut() {
            ...
            tracing::info!(count, "imported SSH hosts from OpenSSH config (Settings button)");
        }
        ...
        for state in &mut self.windows {
            ...
            state.renderer.prune_bg_fx_emitters(
                self.config.background_fx.band_lifetime_secs,
            );
            ...
            if sb_cfg.enabled {
                ...
                if sb_cfg.has_time_segment() {
                    let remaining = interval
                        .saturating_sub(now.duration_since(state.last_status_bar_tick));
                    next_wake = Some(match next_wake {
            ...
            if state.tab_activity_spinner {
                let any_busy = state.tabs.iter().any(|t| {
                    t.panes.values().any(crate::osc_handlers::pane_is_busy)
                });
                if any_busy {
            ...
            if matches!(state.suggestions.state, suggestions::SuggestionState::Loading) {
                state.suggestions.loading_frame =
                    state.suggestions.loading_frame.wrapping_add(1);
                state.window.request_redraw();
        ...
        {
            let sg_idle = std::time::Duration::from_secs(
                u64::from(self.config.ai.suggestions.idle_secs),
            );
            let sg_trigger = self.config.ai.suggestions.trigger;
            let sg_enabled = self.config.ai.suggestions.enabled;
...
fn render_main(state: &mut RunningState) {
    ...
    // row using the focused pane's current scroll offset.
    {
        ...
        let marks = if state.show_prompt_marks {
            if let (Some(spec), Some(emu)) =
                (specs.get(focused_idx), emu_locks.get(focused_idx))
            {
        ...
        let bar = if state.suggestions.enabled {
            match &state.suggestions.state {
                ...
                SuggestionState::Error(m) => Some(SuggestionBar {
                    state: SuggestionBarState::Error {
                        message: m.clone(),
                    },
                }),
            }
...
fn apply_restore_ratio_inner(
    ...
    match node {
        ...
        PaneNode::Split { direction, ratio: node_ratio, a, b } => {
...
fn handle_copy_mode_input(
    ...
    if let Key::Character(s) = logical {
        match s.as_str() {
            "q" => { exit_copy_mode(state); return true; }
            "y" => { yank_copy_mode(state); return true; }
            _ => {}
        }
    }
    ...
    if let Key::Character(s) = logical {
        match s.as_str() {
            "v" if ctrl => {
                state.copy_mode.toggle_selection(copy_mode::SelectionKind::Block);
                sync_copy_mode_selection(state);
                return true;
            }
            "V" => {
                state.copy_mode.toggle_selection(copy_mode::SelectionKind::Line);
                sync_copy_mode_selection(state);
                return true;
            }
            "v" => {
                state.copy_mode.toggle_selection(copy_mode::SelectionKind::Cell);
                sync_copy_mode_selection(state);
                return true;
            }
            _ => {}
    ...
    let delta = {
        ...
        let mut row_texts: Vec<String> = Vec::with_capacity(rows as usize);
        for r in 0..rows {
            let row_str = emu.text_in_range(
                (0, r),
                (col_count.saturating_sub(1), r),
                scroll,
            );
            row_texts.push(row_str);
        }
...
fn build_menu_entries(state: &RunningState) -> Vec<MenuEntry> {
    ...
    let color_submenu: Vec<MenuEntry> = vec![
        MenuEntry { icon: Some("🔴".into()), label: "Red".into(),    hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorRed.as_u32(),    submenu: None },
        MenuEntry { icon: Some("🟠".into()), label: "Orange".into(), hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorOrange.as_u32(), submenu: None },
        MenuEntry { icon: Some("🟡".into()), label: "Yellow".into(), hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorYellow.as_u32(), submenu: None },
        MenuEntry { icon: Some("🟢".into()), label: "Green".into(),  hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorGreen.as_u32(),  submenu: None },
        MenuEntry { icon: Some("🔵".into()), label: "Cyan".into(),   hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorCyan.as_u32(),   submenu: None },
        MenuEntry { icon: Some("🔷".into()), label: "Blue".into(),   hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorBlue.as_u32(),   submenu: None },
        MenuEntry { icon: Some("🟣".into()), label: "Purple".into(), hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorPurple.as_u32(), submenu: None },
        MenuEntry { icon: Some("🩷".into()), label: "Pink".into(),   hotkey: None, enabled: true, separator_before: false, action_id: MenuAction::TabColorPink.as_u32(),   submenu: None },
        MenuEntry { icon: None,              label: "Clear colour".into(), hotkey: None, enabled: true, separator_before: true,  action_id: MenuAction::ClearTabColor.as_u32(), submenu: None },
    ];
    ...
    let icon_submenu: Vec<MenuEntry> = crate::settings_window::ICON_PRESETS
        ...
        .map(|(label, glyph)| MenuEntry {
            ...
            action_id: TAB_ICON_PICKER_BASE + crate::settings_window::ICON_PRESETS
                .iter()
...
fn dispatch_menu_action(state: &mut RunningState, action_id: u32) {
    ...
    match action {
        ...
        MenuAction::TabColorRed => crate::shortcuts::set_tab_user_color(state, Some([0xe0, 0x50, 0x50])),
        MenuAction::TabColorOrange => crate::shortcuts::set_tab_user_color(state, Some([0xe0, 0x90, 0x30])),
        MenuAction::TabColorYellow => crate::shortcuts::set_tab_user_color(state, Some([0xd0, 0xc0, 0x20])),
        MenuAction::TabColorGreen => crate::shortcuts::set_tab_user_color(state, Some([0x30, 0xc0, 0x60])),
        MenuAction::TabColorCyan => crate::shortcuts::set_tab_user_color(state, Some([0x20, 0xb8, 0xc8])),
        MenuAction::TabColorBlue => crate::shortcuts::set_tab_user_color(state, Some([0x40, 0x80, 0xe0])),
        MenuAction::TabColorPurple => crate::shortcuts::set_tab_user_color(state, Some([0x90, 0x50, 0xe0])),
        MenuAction::TabColorPink => crate::shortcuts::set_tab_user_color(state, Some([0xe0, 0x50, 0xa0])),
...
fn menu_items_all(state: &RunningState) -> Vec<(RichMenuItem, MenuAction)> {
    ...
    vec![
        ...
        // ── Tab pin / colour / icon ──────────────────────────────────────────
        {
            ...
            (
                RichMenuItem {
                    label: if is_pinned { "Unpin tab".into() } else { "Pin tab".into() },
                    icon: None,
                    hotkey: None,
                    enabled: true,
                    separator_before: false,
...
pub(crate) fn push_clipboard_history(state: &mut RunningState, text: String) {
    ...
    if state.clipboard_history_ring.front().is_some_and(|t| t == &text) {
        return;
    }
...
fn configs_identical(a: &Config, b: &Config) -> bool {
    ...
    let bg_img_eq = a.appearance.background_image.path == b.appearance.background_image.path
        ...
        && (a.appearance.background_image.brightness - b.appearance.background_image.brightness)
            .abs()
            < f32::EPSILON
        && (a.appearance.background_image.saturation
            - b.appearance.background_image.saturation)
            .abs()
            < f32::EPSILON
    ...
    let keybinds_eq =
        a.keybinds.quake == b.keybinds.quake
            && shortcuts_eq
...
fn import_openssh_hosts(
    ...
    let new_hosts: Vec<terminale_config::SshHost> =
        parsed.into_iter().map(terminale_config::ParsedSshHost::into_ssh_host).collect();
...
fn import_theme_from_picker(
    ...
    let already_present = config.appearance.all_themes().iter().any(|t| t.name == theme.name);
        ...
        terminale_config::BroadcastScope::AllPanesInWindow => {
            for (tab_idx, tab) in state.tabs.iter().enumerate() {
                for (id, pane) in &tab.panes {
                    ...
                    if let Err(e) = pane.session.write_input(bytes) {
                        tracing::warn!(?e, pane_id = id, tab = tab_idx, "broadcast PTY write failed");
                    }
                }
            }
        }
...
mod tests {
    ...
    fn selection_and_pane_actions_are_terminal_only() {
        for a in [
            ...
            assert!(menu_action_is_terminal_only(a), "{a:?} must be terminal-only");
    ...
    fn ranked_query_filters_and_orders() {
        ...
        let ranked = palette_ranked("tab", PaletteMode::Actions, &sc, "Tokyo Night", &[], &[], &[], &[], &[], &[], &[], &[], &[]);
        ...
        assert_eq!(
            palette_ranked("", PaletteMode::Actions, &sc, "Tokyo Night", &[], &[], &[], &[], &[], &[], &[], &[], &[]).len(),
            PALETTE_ACTIONS.len()
        );
    }
    ...
    fn ssh_hosts_surface_as_palette_rows() {
        ...
        let ranked = palette_ranked("", PaletteMode::Actions, &sc, "Tokyo Night", &[], &hosts, &[], &[], &[], &[], &[], &[], &[]);
    ...
    fn theme_mode_lists_all_themes_incl_user_and_marks_current() {
        ...
        let ranked = palette_ranked("", PaletteMode::Themes, &sc, "Matrix", &names, &[], &[], &[], &[], &[], &[], &[], &[]);
    ...
    fn theme_mode_fuzzy_filters() {
        let sc = terminale_config::ShortcutsConfig::default();
        let names = test_theme_names();
        let ranked = palette_ranked("drac", PaletteMode::Themes, &sc, "Tokyo Night", &names, &[], &[], &[], &[], &[], &[], &[], &[]);
        assert!(
    ...
    fn snap_actions_are_all_in_the_palette() {
        use ShortcutAction::{
            SnapBottom, SnapBottomLeft, SnapBottomRight, SnapCenter, SnapLeft, SnapMaximize,
            SnapRight, SnapTop, SnapTopLeft, SnapTopRight, ShowSnapLayouts,
        };
        for want in [
            SnapTop,
            SnapBottom,
    ...
    fn translate_key_emits_correct_pty_bytes() {
        ...
        let go_app = |code: KeyCode, key: &Key| {
            translate_key(&ModifiersState::empty(), PhysicalKey::Code(code), key, None, true)
        };
        assert_eq!(
    ...
    fn rename_state_distinguishes_tab_and_pane() {
        ...
        assert_eq!(tab_rename.target, RenameTarget::Tab, "tab rename uses Tab target");
        ...
        assert_eq!(pane_rename.target, RenameTarget::Pane(3), "pane rename carries pane id");
        ...
        assert_eq!(group_rename.target, RenameTarget::Group(42), "group rename carries group id");
    ...
    fn profile_submenu_action_ids_are_in_picker_base_range() {
        ...
        let entries: Vec<crate::context_menu_window::MenuEntry> = names
            ...
            .zip(icons.iter())
            .enumerate()
            .map(|(idx, (name, icon))| crate::context_menu_window::MenuEntry {
                icon: icon.clone(),
                label: name.clone(),
                hotkey: None,
                enabled: true,
    ...
    fn two_pane_h() -> Vec<(u32, (f32, f32, f32, f32))> {
        vec![(0, (0.0, 0.0, 100.0, 100.0)), (1, (100.0, 0.0, 100.0, 100.0))]
    }

    /// Helper: build a simple 2-pane vertical layout (top | bottom).
    fn two_pane_v() -> Vec<(u32, (f32, f32, f32, f32))> {
        vec![(0, (0.0, 0.0, 200.0, 100.0)), (1, (0.0, 100.0, 200.0, 100.0))]
    }
    ...
    fn adjacency_right_finds_right_neighbour() {
        ...
        let result = pick_adjacent_pane((0.0, 0.0, 100.0, 100.0), &candidates, PaneDirection::Right);
    ...
    fn adjacency_left_finds_left_neighbour() {
        ...
        let result = pick_adjacent_pane((100.0, 0.0, 100.0, 100.0), &candidates, PaneDirection::Left);
    ...
    fn adjacency_three_pane_picks_nearest() {
        ...
        let candidates: Vec<_> = rects.iter().copied().filter(|(id, _)| *id != 1).collect();
        let result =
            pick_adjacent_pane((100.0, 0.0, 100.0, 100.0), &candidates, PaneDirection::Right);
        assert_eq!(result, Some(2), "Right from middle pane should pick right neighbour");
    }
    ...
    fn find_resize_split_flat_vertical_split() {
        ...
        assert_eq!(r.path, Some(Vec::<bool>::new()), "root split has empty path");
    ...
    fn find_resize_split_wrong_direction_returns_none() {
        ...
        assert!(
            find_resize_split(&tree, 0, PaneDirection::Down).is_none(),
        );
    }
    ...
    fn find_resize_split_nested_prefers_innermost() {
        ...
        assert_eq!(r.path, Some(vec![true]), "inner H-split path is Some([true])");
        assert!(r.focused_in_a, "pane 1 is in a-side of H-split");

        // Resize Right on pane 1 → should find the outer V-split (path=Some([])).
        let r = find_resize_split(&tree, 1, PaneDirection::Right).unwrap();
        assert_eq!(r.path, Some(Vec::<bool>::new()), "outer V-split path is Some([])");
    ...
    fn activate_tab_by_index_is_noop_when_out_of_range() {
        ...
        // returns `None`.
        assert_eq!(tab_jump_index(KeyCode::Digit5, 3), None,
            "ActivateTab5 with 3 tabs should be a no-op");
        assert_eq!(tab_jump_index(KeyCode::Digit8, 5), None,
    ...
    fn previous_active_tab_adjusted_after_close() {
        ...
        assert_eq!(active_tab_after_detach(3, 1, 5), 2,
        ...
        assert_eq!(active_tab_after_detach(1, 3, 5), 1,
    ...
    fn user_event_debug_includes_ssh_connected() {
        ...
        let s = format!("{ev:?}");
        assert!(s.contains("SshConnected"), "Debug output should name the variant");
    }
    ...
    fn assert_offscreen_past_edge(
        ...
        assert_eq!(ow, tw, "offscreen_rect must preserve width for edge {edge:?}");
        assert_eq!(oh, th, "offscreen_rect must preserve height for edge {edge:?}");
    ...
    fn configs_identical_detects_divider_grab_padding_change() {
        ...
        let mut b = terminale_config::Config::default();
        b.appearance.divider_grab_padding_logical =
            a.appearance.divider_grab_padding_logical + 2.0;
        assert!(
            !configs_identical(&a, &b),
    ...
    fn snippets_palette_mode_lists_and_fuzzy_filters() {
        ...
        let names: Vec<(String, String)> = vec![
            ("Git status".to_string(), "Show working-tree status".to_string()),
            ("Docker ps".to_string(), "List running containers".to_string()),
            ("Cargo test".to_string(), String::new()),
        ];
        // Empty query lists all snippets.
        let all = palette_ranked("", PaletteMode::Snippets, &sc, "", &[], &[], &names, &[], &[], &[], &[], &[], &[]);
        ...
        let filtered = palette_ranked("git", PaletteMode::Snippets, &sc, "", &[], &[], &names, &[], &[], &[], &[], &[], &[]);
        ...
        let first = palette_ranked("git", PaletteMode::Snippets, &sc, "", &[], &[], &names, &[], &[], &[], &[], &[], &[]);
        assert_eq!(
            first[0].1.binding,
        ...
        let none = palette_ranked("zzz", PaletteMode::Snippets, &sc, "", &[], &[], &names, &[], &[], &[], &[], &[], &[]);
        assert!(none.is_empty(), "non-matching query must yield empty list");

        // Correct index is assigned even when a snippet is not first.
        let docker = palette_ranked("docker", PaletteMode::Snippets, &sc, "", &[], &[], &names, &[], &[], &[], &[], &[], &[]);
        assert!(
            matches!(docker.first().map(|(it, _)| it), Some(PaletteItem::InsertSnippet(1))),
    ...
    fn ring_push_dedupes_consecutive_identical() {
        ...
        ring_push(&mut ring, "same".to_string(), true, 10);
        ring_push(&mut ring, "same".to_string(), true, 10);
        assert_eq!(ring.len(), 1, "consecutive identical entries must be deduped to 1");
    }
    ...
    fn ring_osc52_capture_gate() {
        ...
        assert!(ring.is_empty(), "OSC 52 text must not be captured when gate is false");
    }
    ...
    fn ring_paste_payload_honours_bracketed_paste() {
        ...
        assert!(bracketed.starts_with(b"\x1b[200~"), "must start with bracketed paste start");
        assert!(bracketed.ends_with(b"\x1b[201~"), "must end with bracketed paste end");
    }
    ...
    fn pinned_tab_width_in_appearance_config() {
        ...
        assert!((terminale_config::AppearanceConfig::default().pinned_tab_width - 44.0).abs() < f32::EPSILON);
    }
    ...
    fn pane_swap_rotate_actions_in_palette() {
        let palette_actions: Vec<ShortcutAction> =
            crate::palette::PALETTE_ACTIONS.iter().map(|(a, _)| *a).collect();
        for action in [
    ...
    fn pane_swap_rotate_action_from_name() {
        assert_eq!(crate::keymap::action_from_name("movepaneleft"),  Some(ShortcutAction::MovePaneLeft));
        assert_eq!(crate::keymap::action_from_name("movepaneright"), Some(ShortcutAction::MovePaneRight));
        assert_eq!(crate::keymap::action_from_name("movepaneup"),    Some(ShortcutAction::MovePaneUp));
        assert_eq!(crate::keymap::action_from_name("movepanedown"),  Some(ShortcutAction::MovePaneDown));
        assert_eq!(crate::keymap::action_from_name("rotatepanes"),   Some(ShortcutAction::RotatePanes));
        assert_eq!(crate::keymap::action_from_name("rotatepanesback"), Some(ShortcutAction::RotatePanesBack));
    }
    ...
    fn vert_left_bar(id: WindowId, win_x: i32, win_y: i32, win_w: u32, win_h: u32, strip_w: f32) -> BarRect {
        BarRect {
            id,
            x: win_x,
            y: win_y,
    ...
    fn vert_right_bar(id: WindowId, win_x: i32, win_y: i32, win_w: u32, win_h: u32, strip_w: f32) -> BarRect {
