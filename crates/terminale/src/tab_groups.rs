...
use crate::{
    tabs::refresh_tab_bar, RunningState, TabGroup, TabGroupId, GROUP_COLOR_PALETTE,
};
...
fn next_group_id_after(
    current: Option<TabGroupId>,
    groups: &[TabGroup],
) -> TabGroupId {
    if groups.is_empty() {
...
mod tests {
    ...
    fn next_group_id_after_wraps() {
        let groups = vec![
            TabGroup { id: 10, name: "A".into(), color: [0; 3] },
            TabGroup { id: 20, name: "B".into(), color: [0; 3] },
            TabGroup { id: 30, name: "C".into(), color: [0; 3] },
        ];
        // None → first
    ...
    fn next_group_id_after_unknown_falls_back() {
        let groups = vec![
            TabGroup { id: 5, name: "X".into(), color: [0; 3] },
        ];
    ...
    fn rename_group_updates_name() {
        let mut groups = vec![
            TabGroup { id: 1, name: "Alpha".into(), color: [0; 3] },
            TabGroup { id: 2, name: "Beta".into(), color: [0; 3] },
        ];
    ...
    fn rename_group_empty_name_is_kept() {
        ...
        let mut groups = vec![
            TabGroup { id: 5, name: "Deploy".into(), color: [0; 3] },
        ];
        ...
        assert_eq!(groups[0].name, "Deploy", "empty name must leave original intact");
    }
