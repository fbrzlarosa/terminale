...
use crate::{RunningState, RowsScroll, ShortcutAction};
...
pub(crate) fn pressed_key_name(physical: PhysicalKey, logical: &winit::keyboard::Key) -> Option<String> {
...
pub(crate) fn resolve_shortcut(
    ...
    let table: [(&str, ShortcutAction); 100] = [
        ...
        (sc.open_failed_command_picker.as_str(), OpenFailedCommandPicker),
    ];
...
pub(crate) fn dispatch_shortcut(state: &mut RunningState, action: ShortcutAction) {
    ...
    match action {
        ...
        Paste => {
            match crate::paste_clipboard(state) {
...
pub(crate) fn copy_block_output(state: &mut RunningState) {
    ...
    let block = emu.command_block_at_line(cursor_abs).or_else(|| emu.last_command_block());
...
pub(crate) fn binding_for(action: ShortcutAction, sc: &terminale_config::ShortcutsConfig) -> String {
    use ShortcutAction::*;
    match action {
...
pub(crate) fn scrollback_export_filename(ts: &chrono::DateTime<chrono::Local>) -> String {
    ts.format("terminale-scrollback-%Y%m%d-%H%M%S.txt").to_string()
}
...
pub(crate) fn named_key_with_modifiers(
    ...
    match named {
        ...
        NamedKey::F5 => Some(fn_key_tilde(15, if modified { Some(mod_param) } else { None })),
        NamedKey::F6 => Some(fn_key_tilde(17, if modified { Some(mod_param) } else { None })),
        NamedKey::F7 => Some(fn_key_tilde(18, if modified { Some(mod_param) } else { None })),
        NamedKey::F8 => Some(fn_key_tilde(19, if modified { Some(mod_param) } else { None })),
        NamedKey::F9 => Some(fn_key_tilde(20, if modified { Some(mod_param) } else { None })),
        NamedKey::F10 => Some(fn_key_tilde(21, if modified { Some(mod_param) } else { None })),
        NamedKey::F11 => Some(fn_key_tilde(23, if modified { Some(mod_param) } else { None })),
        NamedKey::F12 => Some(fn_key_tilde(24, if modified { Some(mod_param) } else { None })),
...
pub(crate) fn handle_scroll(state: &mut RunningState, delta: winit::event::MouseScrollDelta) {
    ...
    if alt_screen {
        let bytes = scroll_to_bytes(delta, state.touchpad_pixels_per_row, state.alt_screen_scroll_lines);
        if !bytes.is_empty() {
            if let Some(tab) = state.tabs.get(active) {
...
mod tests {
    use super::{
        binding_for, build_fix_prompt, build_scrollback_export_content,
    ...
    fn plain_char_is_still_typed() {
        ...
        assert_eq!(bytes, Some(b"1".to_vec()), "unmodified \"1\" must type \"1\"");
    }
    ...
    fn extract_empty_when_span_before_buffer() {
        ...
        assert_eq!(out, "line 0", "negative abs lines clamp to the buffer start");
    }
    ...
    fn extract_block_output_text_no_truncation() {
        ...
        assert!(!out.contains("truncated"), "extract_block_output_text must not truncate");
    ...
    fn edit_payload_no_clear_line_omits_ctrl_u() {
        ...
        payload.extend_from_slice(cmd.as_bytes());
        assert_eq!(payload, b"ls -la", "without clears_line, payload is just the command");
        assert!(!payload.ends_with(b"\n"), "no trailing newline");
    }
    ...
    fn block_copy_shortcuts_default_unbound() {
        ...
        assert!(sc.copy_last_command_output.is_empty(), "copy_last_command_output must default to unbound");
        assert!(sc.copy_block_output.is_empty(), "copy_block_output must default to unbound");
        assert!(sc.copy_last_command.is_empty(), "copy_last_command must default to unbound");
        assert!(sc.rerun_last_command.is_empty(), "rerun_last_command must default to unbound");
        assert!(sc.edit_last_command.is_empty(), "edit_last_command must default to unbound");
    }
    ...
    fn export_content_joins_and_trims_trailing_blank_lines() {
        ...
        let content = build_scrollback_export_content(&lines);
        assert_eq!(content, "line one\nline two", "trailing blank lines must be stripped");
    }
    ...
    fn export_content_all_empty_lines_gives_empty() {
        ...
        let content = build_scrollback_export_content(&lines);
        assert!(content.is_empty(), "all-empty buffer must export as empty string");
    }
    ...
    fn export_content_preserves_interior_blank_lines() {
        ...
        let content = build_scrollback_export_content(&lines);
        assert_eq!(content, "first\n\nthird", "interior blank lines must be preserved");
    }
    ...
    fn scrollback_export_dir_roundtrips_toml() {
        let mut cfg = terminale_config::Config::default();
        cfg.terminal.scrollback_export_dir =
    ...
    fn smooth_accumulator_direction_reversal() {
        ...
        assert_eq!(rows, 0, "opposite direction cancels the accumulated remainder");
        assert!(rem.abs() < 1e-4, "remainder must be zero after cancellation, got {rem}");
    }
    ...
    fn smooth_accumulator_large_line_delta() {
        ...
        assert!(rem.abs() < 1e-5, "no remainder for exact integer, got {rem}");
    }
    ...
    fn touchpad_pixels_per_row_validation() {
        ...
        assert!(cfg.validate().is_err(), "value > 128.0 must fail validation");
