...
pub(crate) fn cursor_icon_for_resize(dir: winit::window::ResizeDirection) -> winit::window::CursorIcon {
...
mod tests {
    ...
    fn resolve_button_name_case_insensitive() {
        ...
        let bindings = vec![make_binding("right", "", 1, "Copy")];
        let result = resolve_mouse_binding(MouseButton::Right, &no_mods(), 1, &bindings);
        assert!(result.is_some(), "button name must be matched case-insensitively");
    }
    ...
    fn resolve_wrong_count_returns_none() {
        ...
        let result = resolve_mouse_binding(MouseButton::Right, &no_mods(), 1, &bindings);
        assert!(result.is_none(), "single click must NOT match a double-click binding");
    }
    ...
    fn send_action_bytes_decoded_correctly() {
        ...
        assert!(bytes.starts_with(b"ls -la"), "text prefix must be preserved");
    }
