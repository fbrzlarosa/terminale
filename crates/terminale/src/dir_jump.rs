...
mod tests {
    ...
    fn record_caps_at_max_tracked() {
        ...
        assert!(
            store.len() <= 5,
            "store must respect max_tracked={} but len={}",
            5,
            store.len()
        );
    }
    ...
    fn evict_removes_lowest_frecency() {
        ...
        assert_eq!(
            store.len(),
            5,
            "store must not exceed max_tracked after record"
        );
    }
    ...
    fn save_load_roundtrip() {
        let dir =
            std::env::temp_dir().join(format!("terminale_dirjump_test_{}", std::process::id()));
