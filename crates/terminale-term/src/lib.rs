...
impl Emulator {
    ...
    pub fn lookup_palette_color(&self, index: usize) -> Option<Rgb> {
        ...
        match index {
            ...
            258 => Some(to_rgb(self.override_cursor.unwrap_or(self.palette.foreground))),
            _ => None,
        }
    }
        ...
        for ev in osc133_events {
            if ev.kind == OscKind::OutputStart {
                ...
                let text = if let Some(inline) = ev.inline_command_text.filter(|t| !t.is_empty()) {
                    ...
                    let b_line = self.semantic.pending_command_start_line().unwrap_or(cursor_abs);
                    let cols = self.term.columns();
                    extract_line_text(self.term.grid(), b_line, cols)
                };
                let cwd = self.current_dir.clone();
                self.semantic.record_with_text(ev.kind, ev.line, None, text, cwd);
            } else {
    ...
    pub fn clear_buffer_to_blank(&mut self) {
        ...
        self.parser.advance(&mut self.term, 0x1b);
        self.parser.advance(&mut self.term, b'[');
        self.parser.advance(&mut self.term, b'H');
    }
...
fn snapshot_cell_with_palette(
    ...
    let underline_color = cell
        .underline_color()
        .map(|c| resolve_color(c, eff_fg_default, palette, overrides, override_fg, override_bg));
    CellSnapshot {
        ch: cell.c,
        fg: resolve_color(cell.fg, eff_fg_default, palette, overrides, override_fg, override_bg),
        bg: resolve_color(cell.bg, eff_bg_default, palette, overrides, override_fg, override_bg),
...
fn resolve_color(
    ...
    match color {
        ...
        Color::Named(named) => named_color_with_overrides(named, default, palette, override_fg, override_bg),
...
fn parse_dim_spec(s: &str) -> Option<u16> {
    ...
    let digits = s.trim_end_matches("px").trim_end_matches("Px").trim_end_matches("PX");
    digits.parse::<u16>().ok()
}
...
fn sniff_osc_palette(
    ...
    while i + 3 <= bytes.len() {
        ...
        let payload = if let Ok(s) = std::str::from_utf8(&bytes[payload_start..payload_end]) { s } else {
            i = total_end;
            continue;
        };
...
mod tests {
    ...
    fn underline_style_none_when_no_flags() {
        assert_eq!(UnderlineStyle::from_flags(Flags::empty()), UnderlineStyle::None);
    }

    #[test]
    fn underline_style_single() {
        assert_eq!(UnderlineStyle::from_flags(Flags::UNDERLINE), UnderlineStyle::Single);
    }

    #[test]
    fn underline_style_double() {
        assert_eq!(UnderlineStyle::from_flags(Flags::DOUBLE_UNDERLINE), UnderlineStyle::Double);
    }

    #[test]
    fn underline_style_curly_from_undercurl() {
        assert_eq!(UnderlineStyle::from_flags(Flags::UNDERCURL), UnderlineStyle::Curly);
    }

    #[test]
    fn underline_style_dotted() {
        assert_eq!(UnderlineStyle::from_flags(Flags::DOTTED_UNDERLINE), UnderlineStyle::Dotted);
    }

    #[test]
    fn underline_style_dashed() {
        assert_eq!(UnderlineStyle::from_flags(Flags::DASHED_UNDERLINE), UnderlineStyle::Dashed);
    }
    ...
    fn strikethrough_mapped_from_sgr_advance() {
        ...
        assert!(found, "cell written with SGR 9 must have strikethrough=true");
    }
    ...
    fn underline_color_from_sgr_58() {
        ...
        if let Some(c) = ul_color {
            assert_eq!(c, [255, 128, 0], "underline colour must match SGR 58:2::255:128:0");
        }
    }
    ...
    fn reset_dynamic_colors_clears_all_overrides() {
        ...
        assert_eq!(emu.override_fg(), None);
        assert_eq!(emu.override_bg(), None);
        assert_eq!(emu.lookup_palette_color(0).unwrap().r, emu.palette().normal[0][0]);
    }
    ...
    fn osc_override_fg_bg_affects_cell_snapshot() {
        ...
        assert_eq!(fg, [0xde, 0xad, 0xbe], "OSC 10 fg override must affect cell snapshot");
    }
    ...
    fn decset_2026_timeout_releases_frame() {
        ...
        emu.sync_start = Some(Instant::now().checked_sub(SYNC_OUTPUT_TIMEOUT + Duration::from_millis(1)).unwrap());

        assert!(!emu.is_synchronized_output(), "timed-out sync must not block");
    ...
    fn osc1337_image_placement_via_emulator() {
        ...
        assert!(!placements.is_empty(), "placement must be visible in the viewport");
    }
    ...
    fn clipboard_store_events(bytes: &[u8]) -> Vec<(String, String)> {
        ...
        emu.drain_events()
            .into_iter()
            .filter_map(|e| match e {
                EmulatorEvent::ClipboardStore { kind: _, text } => Some(("clipboard".to_string(), text)),
                _ => None,
            })
            .collect()
    }
    ...
    fn osc52_store_form_not_misclassified_as_read() {
        ...
        assert_eq!(store_events.len(), 1, "store form must produce a ClipboardStore event");
    ...
    fn osc52_read_multiple_queries_detected() {
        ...
        assert_eq!(events.len(), 2, "both ClipboardRead events must be detected");
    ...
    fn osc52_read_empty_selection_accepted() {
        ...
        assert_eq!(events.len(), 1, "ClipboardRead with empty selection must be detected");
    ...
    fn osc52_read_unterminated_sequence_ignored() {
        ...
        assert!(events.is_empty(), "unterminated sequence must not produce an event");
    }
    ...
    fn osc52_mixed_store_then_read() {
        ...
        assert_eq!(read_events.len(), 1, "exactly one ClipboardRead from the query");
    ...
    fn osc133_inline_command_text_captured_single_advance() {
        ...
        assert_eq!(blocks.len(), 1, "one complete A→B→C→D cycle must produce one block");
        assert_eq!(
        ...
        assert!(blocks[0].end_line.is_some(), "block must be finalised after D");
    }
    ...
    fn osc133_cmdhistory_demo_seed_produces_five_blocks() {
        ...
        assert_eq!(blocks[3].command_text, "cargo clippy --workspace --all-targets --all-features");
    ...
    fn osc133_blocks_disabled_inline_command_not_stored() {
        ...
        emu.advance(
            b"\x1b]133;A\x1b\\\x1b]133;B\x1b\\ls\x1b]133;C\x1b\\\x1b]133;D;0\x1b\\"
        );
    ...
    fn visible_lines_text_row_count() {
        ...
        assert_eq!(lines.len(), 24, "visible_lines_text must return screen_lines rows");
    }
