    ...
    match ctrl.format {
        ...
        100 => {
            let img = image::load_from_memory(raw)
                .map_err(
                    |e| tracing::warn!(error = ?e, "apc_graphics: container image decode failed"),
                )
                .ok()?;
...
mod tests {
    ...
    fn parse_control_all_known_keys() {
        let ctrl = parse_control("a=t,f=100,t=d,s=640,v=480,c=80,r=24,i=42,p=7,m=1,o=z");
    ...
    fn parse_control_action_variants() {
        assert_eq!(parse_control("a=T").action, ApcAction::TransmitAndDisplay);
