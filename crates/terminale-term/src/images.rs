...
impl ImageStore {
    ...
    pub fn placements_in_view(
        ...
        for p in &self.placements {
            if p.anchor_line >= top_abs_line && p.anchor_line < bottom {
                let viewport_row = u16::try_from(p.anchor_line - top_abs_line).unwrap_or(u16::MAX);
        ...
        out.sort_by_key(|v| {
            (
                v.placement.anchor_line,
                v.placement.anchor_col,
                v.placement.z,
            )
        });
        out
    ...
    pub fn prune_placements(&mut self, oldest_line: i32) {
        self.placements.retain(|p| p.anchor_line >= oldest_line);
    }
...
mod tests {
    ...
    fn add_rgba8_roundtrip() {
        ...
        let id = store
            .add_rgba8(2, 2, rgba.clone())
            .expect("add_rgba8 must succeed");
    ...
    fn add_rgba8_rejects_zero_dims() {
        let mut store = ImageStore::new();
        assert!(
            store.add_rgba8(0, 1, vec![]).is_none(),
            "zero width must fail"
        );
        assert!(
            store.add_rgba8(1, 0, vec![]).is_none(),
