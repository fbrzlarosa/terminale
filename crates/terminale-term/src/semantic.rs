...
impl SemanticModel {
        ...
        match kind {
            ...
            OscKind::CommandEnd => {
                ...
                if let Some(p) = self.pending.take() {
                    ...
                    self.marks.push(PromptMark { line, exit_code });
                }
            }
        }
    }
    ...
    /// output-activity heuristic on `Pane` handles those shells.
    #[must_use]
...
mod tests {
    ...
    fn blocks_disabled_by_default() {
        ...
        assert!(
            m.blocks().is_empty(),
            "blocks should be empty when max_blocks == 0"
        );
    }
    ...
    fn block_at_line_finds_correct_block() {
        ...
        assert!(
            m.block_at_line(7).is_none(),
            "gap line must not match any block"
        );
    }
    ...
    fn max_blocks_cap_evicts_oldest() {
        ...
        assert_eq!(
            m.blocks().len(),
            3,
            "only 3 newest blocks should be retained"
        );
    ...
    fn prune_drops_blocks_scrolled_off() {
        ...
        assert_eq!(
            m.blocks().len(),
            2,
            "only blocks with end_line >= -20 survive"
        );
    ...
    fn is_command_running_false_before_any_sequence() {
        ...
        assert!(
            ...
            "idle model must not report running"
        );
    }
    ...
    fn is_command_running_false_for_orphan_d() {
        ...
        assert!(
            !m.is_command_running(),
            "orphan D must not leave running state"
        );
    }
    ...
    fn d_without_a_does_not_create_orphan_block() {
        ...
        assert!(
            m.blocks().is_empty(),
            "no block should be created without A"
        );
    ...
    fn failed_filter_selects_only_nonzero_exit_blocks() {
        ...
        assert_eq!(
            failed,
            vec![10, 30],
            "only blocks with non-zero exit should be selected"
        );
    }
    ...
    fn failed_prev_from_middle_of_viewport() {
        ...
        assert_eq!(
            found,
            Some(10),
            "prev failed from 25 must be the block at line 10"
        );
    }
    ...
    fn failed_next_from_middle_of_viewport() {
        ...
        assert_eq!(
            found,
            Some(30),
            "next failed from bottom=25 must be the block at line 30"
        );
    }
