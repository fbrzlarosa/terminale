...
mod tests {
    ...
    fn suggestion_messages_user_contains_context_line_and_env() {
        ...
        assert!(user_content.contains(ctx), "user message must embed terminal_context");
        assert!(user_content.contains(line), "user message must embed current_line");
        assert!(user_content.contains("windows"), "user message must state the OS");
        assert!(user_content.contains("PowerShell"), "user message must state the shell");
    }
