...
pub use bg_image::{
    apply_hsb_cpu, compute_uv_cpu, BgImageFit, BgImageParams, BgImagePipeline,
};
...
mod suggestion_bar_tests {
    ...
    fn bottom_inset_lifts_the_bar_above_a_status_bar() {
        ...
        assert!(approx(l.bar.y, 600.0 - SUGGESTION_BAR_HEIGHT - STATUS_BAR_HEIGHT));
    }

    #[test]
    fn inject_present_only_when_requested() {
        assert!(suggestion_bar_layout(1000, 600, 1.0, 0.0, true).inject.is_some());
        assert!(suggestion_bar_layout(1000, 600, 1.0, 0.0, false).inject.is_none());
    }
    ...
    fn inject_sits_left_of_dismiss_and_both_inside_bar() {
        ...
        assert!(inj.x + inj.w <= l.dismiss.x, "inject must be left of dismiss");
...
fn cells_for(
    ...
    let usable_w =
        (logical_w - padding * 2.0 - offsets.left - offsets.right).max(cell_w);
...
pub fn enforce_min_contrast(fg: [u8; 3], bg: [u8; 3], target_ratio: f32) -> [u8; 3] {
    ...
    let push_toward: [u8; 3] = if bg_lum < 0.5 { [255, 255, 255] } else { [0, 0, 0] };
...
impl Renderer {
    ...
    /// bottom edge).
    fn build_suggestion_bar(&mut self, scale: f32, quads: &mut Vec<Quad>) -> Vec<(Buffer, [f32; 2])> {
        ...
        quads.push(Quad::new([bx, by], [bw, hairline], [0x7d, 0xa6, 0xff], 0.85));
        ...
        let (raw, cmd_color) = match &body {
            SuggestionBody::Loading(frame) => (
                format!("Thinking{}", ".".repeat((frame % 4) as usize)),
                dim,
            ),
        ...
        // Dismiss ✕, centred in its button.
        {
            ...
            let mut buf = Buffer::new(&mut self.font_system, metrics);
            buf.set_wrap(&mut self.font_system, Wrap::None);
            buf.set_size(&mut self.font_system, Some(layout.dismiss.w * scale), Some(bh));
            buf.set_text(
                &mut self.font_system,
    ...
    fn build_label_overlays(
        ...
        for badge in &badges {
            ...
            let label_len = (badge.typed_prefix.chars().count()
                + badge.remaining.chars().count())
            .max(1) as f32;
            ...
            let fill = if badge.highlighted { badge_bg_hi } else { badge_bg };
    ...
    fn build_snap_chooser(
        ...
        for row in 0..rows {
            for col in 0..cols {
                ...
                let (wx, wy, ww, wh): (f32, f32, f32, f32) = match cell {
                    SnapChooserCell::Left =>
                        (diag_x, diag_y, diag_w * 0.5, diag_h),
                    SnapChooserCell::Right =>
                        (diag_x + diag_w * 0.5, diag_y, diag_w * 0.5, diag_h),
                    SnapChooserCell::Top =>
                        (diag_x, diag_y, diag_w, diag_h * 0.5),
                    SnapChooserCell::Bottom =>
                        (diag_x, diag_y + diag_h * 0.5, diag_w, diag_h * 0.5),
                    SnapChooserCell::TopLeft =>
                        (diag_x, diag_y, diag_w * 0.5, diag_h * 0.5),
                    SnapChooserCell::TopRight =>
                        (diag_x + diag_w * 0.5, diag_y, diag_w * 0.5, diag_h * 0.5),
                    SnapChooserCell::BottomLeft =>
                        (diag_x, diag_y + diag_h * 0.5, diag_w * 0.5, diag_h * 0.5),
                    SnapChooserCell::BottomRight =>
                        (diag_x + diag_w * 0.5, diag_y + diag_h * 0.5, diag_w * 0.5, diag_h * 0.5),
                    SnapChooserCell::Center => {
                        let sw = diag_w * 0.6;
                        let sh = diag_h * 0.6;
                        (diag_x + (diag_w - sw) * 0.5, diag_y + (diag_h - sh) * 0.5, sw, sh)
                    }
                    SnapChooserCell::Maximize =>
                ...
                quads.push(Quad::new([wx, wy], [ww.max(1.0), wh.max(1.0)], preview_col, 0.90));
                ...
                let mut lbuf = Buffer::new(
                    &mut self.font_system,
                    Metrics::new(label_fs, label_h_log),
                );
                lbuf.set_size(
                    &mut self.font_system,
                    Some(cell_w / scale),
        ...
        let mut hint_buf = Buffer::new(
            &mut self.font_system,
            Metrics::new(hint_fs, hint_h_log),
        );
        hint_buf.set_size(
            &mut self.font_system,
            Some(pw / scale),
    ...
    pub fn jump_highlight_active(&self) -> bool {
        self.jump_highlight_band.is_some_and(|(_, alpha)| alpha > 0.0)
    }
    ...
    pub fn snap_chooser_hit(&self, px: f32, py: f32) -> Option<usize> {
        ...
        for row in 0..row_count {
            for col in 0..cols {
                ...
                let cx = overlay_x + SNAP_CHOOSER_PAD * scale
                    + col as f32 * (cell_w + gap);
                let cy = overlay_y + SNAP_CHOOSER_HEADER_H * scale
                    + row as f32 * (cell_h + gap);
    ...
    fn top_offset_logical(&self) -> f32 {
        ...
        let vert_ctrl_top = if self.tab_bar_visible_logical() > 0.0
            && self.tab_bar_placement.is_vertical()
        {
    ...
    fn left_offset_logical(&self) -> f32 {
        if self.tab_bar_visible_logical() > 0.0
            && self.tab_bar_placement == TabBarPlacement::Left
        {
    ...
    fn right_offset_logical(&self) -> f32 {
        if self.tab_bar_visible_logical() > 0.0
            && self.tab_bar_placement == TabBarPlacement::Right
        {
            self.vertical_tab_bar_width
        } else {
            0.0
    ...
    fn vert_ctrl_layout(&self, strip_x: f32, strip_w: f32) -> TabLayout {
        ...
        TabLayout {
            tabs: Vec::new(),
            plus: LogicalRect { x: 0.0, y: 0.0, w: 0.0, h: 0.0 },
            min_btn,
            max_btn,
            close_btn,
    ...
    fn tab_layout(&self, bar: &TabBar) -> TabLayout {
        ...
        let pill_total: f32 = bar.items.iter().map(|item| {
    ...
    pub fn body_top_px(&self) -> f32 {
        self.top_offset_logical() * self.scale_factor
            + self.grid_top_shift_px(self.config.height)
    }
    ...
    pub fn set_font_style_overrides(
        ...
        self.font_bold_family = resolve_override_family(&self.font_system, bold, "font.bold_family");
        self.font_italic_family =
            resolve_override_family(&self.font_system, italic, "font.italic_family");
        self.font_bold_italic_family = resolve_override_family(
            &self.font_system,
            bold_italic,
            "font.bold_italic_family",
        );
    }
    ...
    pub fn render_panes(&mut self, panes: &[PaneSpec<'_>]) -> Result<(), RenderError> {
        ...
        if panes.len() > 1 && self.inactive_pane_dim > 0.01 {
            ...
            for (idx, spec) in panes.iter().enumerate() {
                ...
                self.pane_dim_quads.push(Quad::new(
                    [rx, ry],
                    [rw, rh],
                    [0x00, 0x00, 0x00],
                    alpha,
        ...
        for (row_idx, row_cells) in grid_cells.iter().enumerate() {
            for (col_idx, snap) in row_cells.iter().enumerate() {
                ...
                if style != UnderlineStyle::None {
                    ...
                    emit_underline_quads(&mut tmp, style, x, baseline_y, cw_px, underline_thickness, color);
                    self.extra_pane_quads.extend(tmp);
                }
                // Strikethrough.
                if snap.strikethrough {
                    let x = pane_x + pad_px + (col_idx as f32) * cw_px;
                    let y = pane_y + (row_idx as f32) * ch_px + ch_px * 0.5 - underline_thickness * 0.5;
                    self.extra_pane_quads.push(Quad::new([x, y], [cw_px, underline_thickness], snap.fg, 1.0));
                }
                    ...
                    && !snap.hidden
                {
                    if let Some(rects) = box_drawing::box_rects(snap.ch) {
                        ...
                        for r in rects {
                            ...
                            self.extra_pane_quads.push(Quad::new([qx, qy], [qw, qh], snap.fg, r.alpha));
                        }
                        ' '
                    } else {
                        snap.ch
                    }
                } else if snap.ch == '\0' { ' ' } else { snap.ch };
    ...
    pub fn render(&mut self, emulator: &Emulator) -> Result<(), RenderError> {
        ...
        for (row_idx, row_cells) in grid_cells.iter().enumerate() {
            for (col_idx, snap) in row_cells.iter().enumerate() {
                ...
                if style != UnderlineStyle::None {
                    ...
                    let baseline_y =
                        body_y_origin + (row_idx as f32) * ch_px + ch_px - underline_thickness - 1.0;
                    let color = if snap.has_link {
                        accent
                    } else {
                        snap.underline_color.unwrap_or(snap.fg)
                    };
                    emit_underline_quads(&mut quads, style, x, baseline_y, cw_px, underline_thickness, color);
                }
                // Strikethrough: mid-cell horizontal bar (SGR 9).
                if snap.strikethrough {
                    let x = body_x_origin + pad_px + (col_idx as f32) * cw_px;
                    let y = body_y_origin + (row_idx as f32) * ch_px + ch_px * 0.5 - underline_thickness * 0.5;
                    quads.push(Quad::new([x, y], [cw_px, underline_thickness], snap.fg, 1.0));
                }
        ...
        if !self.prompt_marks.is_empty() {
            ...
            for &(row, exit_code) in &self.prompt_marks {
                ...
                let dot_y = body_y_origin + (row_idx as f32) * ch_px
                    + (ch_px - dot_size_px) * 0.5;
        ...
        if let Some(bar) = &tab_bar_snapshot {
            ...
            if self.tab_bar_placement.is_vertical() {
                self.render_vertical_tab_strip(
                    bar,
                    scale,
                    &mut quads,
                    &mut tab_text_areas,
            ...
            let tab_bar_y_px =
                self.tab_bar_y_logical(self.config.height as f32) * scale;
            ...
            let sep_y = match self.tab_bar_placement {
                ...
                TabBarPlacement::Bottom | TabBarPlacement::Left | TabBarPlacement::Right => {
                    tab_bar_y_px
                }
            };
            quads.push(Quad::new(
                ...
                tab_rects.iter().zip(&bar.items).enumerate()
            {
                ...
                if let Some(tint) = item.color {
                    quads.push(Quad::new(
                        [tab_rect.x * scale, (tab_y_log + tab_rect.y + tab_rect.h - 2.0) * scale],
                ...
                if let Some(ga) = item.group_accent {
                    ...
                    if is_run_end {
                        quads.push(Quad::new(
                            [(tab_rect.x + tab_rect.w - 3.0) * scale, (tab_y_log + tab_rect.y) * scale],
                            [3.0 * scale, tab_rect.h * scale],
                            ga,
                            1.0,
                        ));
                ...
                if item.pinned {
                    ...
                    let pin_y = tab_y_log + tab_rect.y + (tab_rect.h - self.font_size * 1.1) * 0.5;
                    ...
                    let text_y = tab_y_log + tab_rect.y + (tab_rect.h - self.font_size * 1.1) * 0.5;
                    tab_text_areas.push((buf, [text_x, text_y]));
            ...
            if self.show_tab_group_labels {
                for &(pill_rect, first_idx) in &layout.group_pills {
                    let accent = bar.items[first_idx].group_accent.unwrap_or([0x4e, 0xa8, 0xff]);
                    ...
                    if let Some(ref glabel) = bar.items[first_idx].group_label {
                        ...
                        let text_y = tab_y_log + pill_rect.y
            ...
            tab_text_areas.push((plus_buf, [plus_rect.x + 12.0, tab_y_log + plus_rect.y + 6.0]));
        ...
        if let Some(sb) = &self.status_bar {
            ...
            let sep_y = if sb.at_bottom { bar_y } else { bar_y + bar_h - 1.0 * scale };
            quads.push(Quad::new(
                [0.0, sep_y],
                [surface_w, 1.0 * scale],
                [0x22, 0x28, 0x3a],
            ...
            if !sb.right.is_empty() {
                ...
                let est_w_px =
                    sb.right.chars().count() as f32 * self.cell_width * 0.6 * scale;
        ...
        if self.bg_image_params.active() {
            ...
            self.bg_image.upload(&self.device, &self.queue, viewport_px, &params);
        }
        ...
        let images_to_upload: Vec<_> = emulator
            ...
            .map(|img| (img.id, img.rgba.clone(), img.width_px, img.height_px, img.byte_size))
            .collect();
        for (id, rgba, width_px, height_px, byte_size) in images_to_upload {
            let proxy = terminale_term::InlineImage { id, rgba, width_px, height_px, byte_size };
            self.image_blit.upload_image(&self.device, &self.queue, &proxy);
        }
...
static TABLER_ICONS_SUBSET: &[u8] =
    include_bytes!("../assets/fonts/icons/TablerIcons-subset.ttf");
        ...
        .filter(|face| {
            face.monospaced
                || face.families.iter().any(|(n, _)| {
                    bundled.iter().any(|b| n.eq_ignore_ascii_case(b))
                })
        })
...
fn emit_underline_quads(
    ...
    match style {
        ...
        UnderlineStyle::Double => {
            ...
            let gap = t * 2.0;
            quads.push(Quad::new([x, baseline_y], [cell_w, t], color, 1.0));
            quads.push(Quad::new([x, baseline_y - gap - t], [cell_w, t], color, 1.0));
        }
...
fn compute_pill_geometry_pure(
    ...
    for (idx, &(has_label, char_count)) in items.iter().enumerate() {
        if has_label {
            let pill_w = (GROUP_PILL_PAD_X * 2.0
                + font_size * 0.62 * 0.72 * char_count as f32)
                .clamp(24.0, 140.0);
            let pill_rect = LogicalRect {
                x: x + GROUP_PILL_GAP,
                y,
...
mod tests {
    ...
    fn bundled_bold_weight_resolves() {
        ...
        let run = buf
            .layout_runs()
            .next()
            .expect("at least one layout run");
    ...
    fn cells_for_subtracts_padding_and_floors() {
        ...
        assert_eq!(
            cells_for(
                ...
                16.0,
                8.0,
                GridOffsets { top: 44.0, bottom: 8.0, left: 0.0, right: 0.0 }
            ),
            (126, 34)
        );
        ...
        assert_eq!(
            cells_for(
                ...
                10.0,
                4.0,
                GridOffsets { top: 20.0, bottom: 4.0, left: 0.0, right: 0.0 }
            ),
            (10, 7)
        );
    ...
    fn cells_for_vertical_strip_shrinks_cols() {
        ...
        let (cols, _) = cells_for(
            ...
            16.0,
            8.0,
            GridOffsets { top: 44.0, bottom: 8.0, left: 180.0, right: 0.0 },
        );
        ...
        let (cols_r, _) = cells_for(
            ...
            16.0,
            8.0,
            GridOffsets { top: 44.0, bottom: 8.0, left: 0.0, right: 180.0 },
        );
    ...
    fn cells_for_status_bar_bottom_shrinks_rows() {
        ...
        let (_, rows_no_bar) = cells_for(
            ...
            20.0,
            4.0,
            GridOffsets { top: 40.0, bottom: 4.0, left: 0.0, right: 0.0 },
        );
    ...
    fn truncate_tab_title_long_gets_ellipsis() {
        ...
        assert!(result.ends_with('\u{2026}'), "must end with ellipsis: {result:?}");
    ...
    fn apply_cell_width_multiplier_clamps_high() {
        ...
        assert!((result - 20.0).abs() < 1e-4, "should clamp to 2.0: {result}");
    }
    ...
    fn status_bar_right_tx_clamps_to_zero() {
        ...
        assert!(tx.abs() < f32::EPSILON, "must clamp to zero, not go negative");
    }
    ...
    fn vertical_strip_left_shrinks_columns() {
        ...
        let (cols_no_strip, _) = cells_for(
            ...
            20.0,
            4.0,
            GridOffsets { top: 4.0, bottom: 4.0, left: 0.0, right: 0.0 },
        );
        ...
        let (cols_with_strip, _) = cells_for(
            ...
            20.0,
            4.0,
            GridOffsets { top: 4.0, bottom: 4.0, left: 180.0, right: 0.0 },
        );
        assert_eq!(
    ...
    fn vertical_strip_right_shrinks_columns() {
        ...
        let (cols_with_strip, _) = cells_for(
            ...
            20.0,
            4.0,
            GridOffsets { top: 4.0, bottom: 4.0, left: 0.0, right: 180.0 },
        );
        assert_eq!(
            cols_no_strip as i32 - cols_with_strip as i32,
            18,
    ...
    fn lerp_u8_midpoint() {
        ...
        assert!(result == 127 || result == 128, "midpoint lerp should be ~128, got {result}");
    }
    ...
    fn apply_inverse_then_dim() {
        ...
        let out = apply_sgr_attributes(snap, 1.0);
        assert_eq!(out.fg, [200, 200, 200], "dim after inverse should produce fg==new-bg");
    ...
    fn wcag_contrast_white_on_black_is_21() {
        ...
        assert!((r - 21.0).abs() < 0.1, "white-on-black must be ~21:1, got {r}");
    }
    ...
    fn wcag_contrast_identical_colours_is_1() {
        ...
        assert!((r - 1.0).abs() < 0.01, "identical colours must be 1:1, got {r}");
    }
    ...
    fn enforce_min_contrast_ratio_one_is_noop() {
        ...
        assert_eq!(enforce_min_contrast(fg, bg, 1.0), fg, "ratio=1.0 must be a no-op");
    }
    ...
    fn select_font_family_bold_italic_falls_back_to_bold_override() {
        ...
        assert!(italic_s, "italic synthesis still applied on top of bold face");
    }
    ...
    fn select_font_family_bold_italic_falls_back_to_italic_override() {
        ...
        assert!(!italic_s, "dedicated italic face; slant synthesis not needed");
    }
    ...
    fn pane_dim_emits_one_quad_for_non_focused_pane() {
        ...
        assert_eq!(quads.len(), 1, "exactly one overlay quad for the non-focused pane");
        assert_eq!(quads[0], right, "quad must cover the non-focused pane's rect exactly");
    }
    ...
    fn pane_dim_emits_two_quads_for_three_pane_split_focus_middle() {
        ...
        assert_eq!(quads.len(), 2, "two overlay quads for two non-focused panes");
    ...
    fn vertical_remainder_is_sub_cell() {
        ...
        assert!((rem - 16.0).abs() < 0.01, "expected 16px leftover, got {rem}");
        assert!((0.0..20.0).contains(&rem), "remainder must be in [0, cell_h)");
    }
    ...
    fn vertical_remainder_always_less_than_cell_h() {
        ...
        assert!(exact.abs() < 0.01, "exact fit must leave 0 remainder, got {exact}");
        // Non-zero remainder must still be in [0, cell_h).
        let odd = vertical_remainder(607.0, 20.0, 40.0, 4.0); // usable=563, rows=28, leftover=3
        assert!((0.0..20.0).contains(&odd), "remainder must be in [0, cell_h), got {odd}");
    }
    ...
    fn grid_centering_splits_remainder_symmetrically() {
        ...
        let rows = ((surface_h - top_px - bottom_px).max(cell_h_px) / cell_h_px).floor().max(1.0);
    ...
    fn bottom_gap_at_most_one_cell() {
        ...
        let top: f32 = 44.0;
        let bottom: f32 = 8.0;
        let rows = ((surface_h - top - bottom).max(cell_h) / cell_h).floor().max(1.0);
    ...
    fn hidden_bars_contribute_zero_to_offsets() {
        ...
        let (_, rows_top_tab) = cells_for(
            800.0, 600.0, 8.0, 16.0, padding,
            GridOffsets { top: top_with_tab, bottom: bottom_no_bar, left: 0.0, right: 0.0 },
        );
        ...
        let (_, rows_bottom_bar) = cells_for(
            800.0, 600.0, 8.0, 16.0, padding,
            GridOffsets { top: top_with_tab, bottom: bottom_with_bar, left: 0.0, right: 0.0 },
        );
        assert!(
        ...
        let rem = usable % 16.0;
        assert!((0.0..16.0).contains(&rem), "bottom remainder must be in [0, cell_h), got {rem}");
    }
    ...
    fn group_pill_hit_returns_first_idx() {
        ...
        assert!(pill_rect.contains(cx, cy), "centre of pill must be inside the rect");
    }
    ...
    fn slot_from_midpoints_single_tab() {
        ...
        assert_eq!(slot_from_midpoints(&rows, 54.0), 1, "at/past midpoint → 1 (append)");
