...
pub use ssh::{
    dedupe_imported_hosts, default_known_hosts_path, default_openssh_config_path,
    default_ssh_port, HostKeyPolicy, ImportOpenSshConfig, ParsedSshHost, SshAuthMethod, SshConfig,
    SshHost, parse_ssh_config,
};
pub use theme::{builtin_themes, ResolvedTheme, Theme};

// ── Re-exports: domain modules (identical public API as before) ───────────────
pub use ai::{
    AiConfig, AiSuggestionsConfig, ClaudeAiConfig, OllamaAiConfig, OpenAiAiConfig, SuggestionTrigger,
};
pub use appearance::{
...
pub use keybinds::{
    decode_send_string, CustomKeybind, KeyActionSpec, KeybindsConfig, KeyTable, KeyTableEntry,
...
fn render_default_toml(cfg: &Config) -> String {
    ...
    buf.push_str("# Docs: https://stackbyte.dev/terminale/config\n\n");
...
"#;
    ...
    fn underline_thickness_bounds_validate() {
        ...
        assert!(cfg.validate().is_ok(), "0.5 is the minimum and must validate");
        // Exactly at the maximum.
        cfg.font.underline_thickness_px = 4.0;
        assert!(cfg.validate().is_ok(), "4.0 is the maximum and must validate");
        // Too thick.
...
"#;
        ...
        assert_eq!(
            back.keybinds.shortcuts.focus_pane_left,
            "Alt+ArrowLeft"
        );
...
"#;
    ...
    fn status_bar_default_disabled() {
        ...
        cfg.validate().expect("default status_bar config must validate");
    }
    ...
    fn reload_config_shortcut_roundtrip() {
        ...
        let cfg: Config =
            toml::from_str(toml_src).expect("reload_config shortcut must parse");
        cfg.validate().expect("must validate");
        ...
        assert_eq!(
            back.keybinds.shortcuts.reload_config,
            "Ctrl+Shift+F5",
...
"#;
        let cfg: Config =
            toml::from_str(toml_src).expect("old config with [keystroke_fx] and quake.stay_on_top must parse");
        cfg.validate()
...
"#;
        let cfg: KeybindsConfig =
            toml::from_str(toml_src).expect("TOML parse failed");
        ...
        let cfg2: KeybindsConfig =
            toml::from_str(&serialised).expect("re-parse failed");
    ...
    fn custom_keybind_default_is_empty() {
        ...
        assert!(cfg.custom.is_empty(), "default custom keybinds must be empty");
    }
    ...
    fn ssh_config_default_policy_is_accept_new() {
        assert_eq!(Config::default().ssh.host_key_policy, HostKeyPolicy::AcceptNew);
    }
    ...
    fn ssh_config_known_hosts_roundtrips() {
        ...
        cfg.validate().expect("must validate");
        assert_eq!(cfg.ssh.known_hosts, std::path::PathBuf::from("/tmp/my_known_hosts"));
        let s = toml::to_string(&cfg).unwrap();
        let back: Config = toml::from_str(&s).unwrap();
        assert_eq!(back.ssh.known_hosts, std::path::PathBuf::from("/tmp/my_known_hosts"));
    }
    ...
    fn zen_hide_default_is_all_elements() {
        ...
        assert!(cfg.window.zen_hide.iter().any(|e| matches!(e, ZenHideElement::TabBar)));
        assert!(cfg.window.zen_hide.iter().any(|e| matches!(e, ZenHideElement::StatusBar)));
        assert!(cfg.window.zen_hide.iter().any(|e| matches!(e, ZenHideElement::PaneHeaders)));
        assert!(cfg.window.zen_hide.iter().any(|e| matches!(e, ZenHideElement::TitleBar)));
    }
...
"#;
        ...
        let s = toml::to_string(&cfg).unwrap();
        let back: Config = toml::from_str(&s).unwrap();
        back.validate().expect("roundtripped zen config must validate");
...
"#;
        let cfg: Config =
            toml::from_str(toml_src).expect("fullscreen/zen shortcuts must parse");
        cfg.validate().expect("must validate");
...
"#;
    ...
    fn snippet_rejects_empty_name() {
        ...
        assert!(cfg.validate().is_err(), "empty snippet name must be rejected");
    }
    ...
    fn offer_fix_on_failure_parses_and_roundtrips() {
        ...
        let cfg: Config =
            toml::from_str(toml_src).expect("offer_fix_on_failure must parse");
        cfg.validate().expect("must validate");
        assert!(
    ...
    fn fix_last_command_shortcut_defaults_empty() {
        assert!(
            Config::default().keybinds.shortcuts.fix_last_command.is_empty(),
    ...
    fn fix_last_command_shortcut_parses_and_roundtrips() {
        ...
        let cfg: Config =
            toml::from_str(toml_src).expect("fix_last_command binding must parse");
        cfg.validate().expect("must validate");
        assert_eq!(
            cfg.keybinds.shortcuts.fix_last_command,
            "Ctrl+Shift+X",
        ...
        let back: Config = toml::from_str(&serialized).expect("must roundtrip");
        assert_eq!(
            back.keybinds.shortcuts.fix_last_command,
            "Ctrl+Shift+X",
