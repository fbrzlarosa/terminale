    ...
    pub fn as_send_bytes(&self) -> Option<Vec<u8>> {
        match self {
            KeyActionSpec::Action(s) => {
                s.strip_prefix("send:").map(decode_send_string)
            }
        }
    }
...
mod tests {
    ...
    fn mouse_bindings_default_empty() {
        ...
        assert!(
            cfg.mouse.is_empty(),
            "mouse bindings must default to empty"
        );
    }
    ...
    fn mouse_binding_count_default_is_one() {
        ...
        assert_eq!(
            w.mouse[0].count, 1,
            "count must default to 1 when omitted"
        );
    }
    ...
    fn key_tables_default_empty() {
        let cfg = KeybindsConfig::default();
        assert!(cfg.key_tables.is_empty(), "key_tables must default to empty");
    }
    ...
    fn key_table_timed_out_pure_fn() {
        ...
        let timed_out = before.duration_since(base)
        ...
        let timed_out = after.duration_since(base)
            >= std::time::Duration::from_millis(500);
