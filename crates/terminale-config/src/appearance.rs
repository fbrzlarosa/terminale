...
mod tests {
    ...
    fn pane_tear_out_roundtrip() {
        let cfg = AppearanceConfig { pane_tear_out: false, ..Default::default() };
    ...
    fn vertical_tab_bar_width_validation() {
        let bad_low = AppearanceConfig { vertical_tab_bar_width: 50.0, ..Default::default() };
        assert!(bad_low.validate().is_err());
        let bad_high = AppearanceConfig { vertical_tab_bar_width: 400.0, ..Default::default() };
        assert!(bad_high.validate().is_err());
        let good = AppearanceConfig { vertical_tab_bar_width: 200.0, ..Default::default() };
    ...
    fn dim_amount_bounds_validate() {
        // Below 0.0
        let bad_low = AppearanceConfig { dim_amount: -0.1, ..Default::default() };
        assert!(bad_low.validate().is_err(), "-0.1 must be rejected");
        // Above 1.0
        let bad_high = AppearanceConfig { dim_amount: 1.1, ..Default::default() };
        assert!(bad_high.validate().is_err(), "1.1 must be rejected");
        // Boundary values pass
        let zero = AppearanceConfig { dim_amount: 0.0, ..Default::default() };
        assert!(zero.validate().is_ok(), "0.0 must be accepted");
        let one = AppearanceConfig { dim_amount: 1.0, ..Default::default() };
        assert!(one.validate().is_ok(), "1.0 must be accepted");
    }

    #[test]
    fn dim_amount_roundtrips() {
        let cfg = AppearanceConfig { dim_amount: 0.3, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn minimum_contrast_bounds_validate() {
        ...
        let bad_low = AppearanceConfig { minimum_contrast: 0.5, ..Default::default() };
        assert!(bad_low.validate().is_err(), "0.5 must be rejected");
        // Above 21.0 is invalid.
        let bad_high = AppearanceConfig { minimum_contrast: 22.0, ..Default::default() };
        assert!(bad_high.validate().is_err(), "22.0 must be rejected");
        // Boundary values pass.
        let at_one = AppearanceConfig { minimum_contrast: 1.0, ..Default::default() };
        assert!(at_one.validate().is_ok(), "1.0 must be accepted");
        let at_max = AppearanceConfig { minimum_contrast: 21.0, ..Default::default() };
        assert!(at_max.validate().is_ok(), "21.0 must be accepted");
    }

    #[test]
    fn minimum_contrast_roundtrips_toml() {
        let cfg = AppearanceConfig { minimum_contrast: 4.5, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn inactive_pane_dim_bounds_validate() {
        let bad_low = AppearanceConfig { inactive_pane_dim: -0.1, ..Default::default() };
        assert!(bad_low.validate().is_err(), "-0.1 must be rejected");
        let bad_high = AppearanceConfig { inactive_pane_dim: 0.95, ..Default::default() };
        assert!(bad_high.validate().is_err(), "0.95 must be rejected (> 0.9)");
        let zero = AppearanceConfig { inactive_pane_dim: 0.0, ..Default::default() };
        assert!(zero.validate().is_ok(), "0.0 must be accepted");
        let max = AppearanceConfig { inactive_pane_dim: 0.9, ..Default::default() };
        assert!(max.validate().is_ok(), "0.9 must be accepted");
    }

    #[test]
    fn inactive_pane_dim_roundtrips_toml() {
        let cfg = AppearanceConfig { inactive_pane_dim: 0.4, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn unfocused_window_dim_bounds_validate() {
        let bad_low = AppearanceConfig { unfocused_window_dim: -0.1, ..Default::default() };
        assert!(bad_low.validate().is_err(), "-0.1 must be rejected");
        let bad_high = AppearanceConfig { unfocused_window_dim: 1.0, ..Default::default() };
        assert!(bad_high.validate().is_err(), "1.0 must be rejected (> 0.9)");
        let zero = AppearanceConfig { unfocused_window_dim: 0.0, ..Default::default() };
        assert!(zero.validate().is_ok(), "0.0 must be accepted");
        let max = AppearanceConfig { unfocused_window_dim: 0.9, ..Default::default() };
        assert!(max.validate().is_ok(), "0.9 must be accepted");
    }

    #[test]
    fn unfocused_window_dim_roundtrips_toml() {
        let cfg = AppearanceConfig { unfocused_window_dim: 0.35, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn pinned_tab_width_bounds_validate() {
        let bad_low = AppearanceConfig { pinned_tab_width: 10.0, ..Default::default() };
        assert!(bad_low.validate().is_err(), "10.0 must be rejected");
        let bad_high = AppearanceConfig { pinned_tab_width: 200.0, ..Default::default() };
        assert!(bad_high.validate().is_err(), "200.0 must be rejected");
        let at_min = AppearanceConfig { pinned_tab_width: 24.0, ..Default::default() };
        assert!(at_min.validate().is_ok(), "24.0 must be accepted");
        let at_max = AppearanceConfig { pinned_tab_width: 120.0, ..Default::default() };
        assert!(at_max.validate().is_ok(), "120.0 must be accepted");
    }

    #[test]
    fn pinned_tab_width_roundtrips_toml() {
        let cfg = AppearanceConfig { pinned_tab_width: 56.0, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn all_themes_deduplicates_by_name_builtin_wins() {
        ...
        assert_ne!(winner.background, "#ff0000", "built-in must win over user-defined");
    }
    ...
    fn scan_themes_dir_skips_invalid_toml() {
        ...
        let themes = scan_themes_dir(dir.path());
        assert!(themes.is_empty(), "invalid TOML must be skipped, not panicked");
    }
    ...
    fn all_themes_merges_dir_themes_with_deduplication() {
        ...
        assert_eq!(collision_count, 1, "colliding drop-in must not create a duplicate");
    ...
    fn builtin_box_drawing_roundtrips_toml() {
        let cfg = AppearanceConfig { builtin_box_drawing: false, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn show_tab_group_labels_roundtrips_toml() {
        let cfg = AppearanceConfig { show_tab_group_labels: false, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn bundled_icons_roundtrips_toml() {
        let cfg = AppearanceConfig { bundled_icons: false, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
    ...
    fn tab_activity_spinner_roundtrips_toml() {
        let cfg = AppearanceConfig { tab_activity_spinner: false, ..Default::default() };
        let toml = toml::to_string(&cfg).unwrap();
        let parsed: AppearanceConfig = toml::from_str(&toml).unwrap();
        assert!(
