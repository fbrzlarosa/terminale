...
mod tests {
    ...
    fn exit_behavior_roundtrip_toml() {
        ...
        for behavior in ExitBehavior::all() {
            ...
            assert_eq!(back.terminal.exit_behavior, behavior,
    ...
    fn default_hyperlink_rules_is_non_empty() {
        ...
        assert!(!rules.is_empty(), "default_hyperlink_rules() must not be empty");
    ...
    fn hyperlink_rules_roundtrip_toml() {
        ...
        for (a, b) in cfg.terminal.hyperlink_rules.iter()
    ...
    fn hyperlink_rules_valid_regex_passes_validate() {
        let mut cfg = TerminalConfig::default();
        cfg.hyperlink_rules.push(HyperlinkRule::new(
            r"https?://\S+",
            "HTTP/HTTPS URL",
        ));
...
"#;
    ...
    fn link_hover_delay_ms_default_is_zero() {
        let cfg = TerminalConfig::default();
        assert_eq!(cfg.link_hover_delay_ms, 0, "link_hover_delay_ms must default to 0");
    }
    ...
    fn link_hover_tooltip_roundtrip_toml() {
        ...
        let raw = toml::to_string(&cfg).expect("serialise");
        let back: crate::Config = toml::from_str(&raw).expect("deserialise");
        assert!(!back.terminal.link_hover_tooltip, "link_hover_tooltip=false must roundtrip");
        assert_eq!(back.terminal.link_hover_delay_ms, 500, "link_hover_delay_ms=500 must roundtrip");
    }

    #[test]
    fn link_hover_delay_ms_validation() {
        // 2001 is out of range.
        let bad = TerminalConfig { link_hover_delay_ms: 2001, ..Default::default() };
        assert!(bad.validate().is_err(), "2001 ms must be rejected");
        // 2000 is the max and must pass.
        let at_max = TerminalConfig { link_hover_delay_ms: 2000, ..Default::default() };
        assert!(at_max.validate().is_ok(), "2000 ms must be accepted");
        // 0 must pass.
        let at_zero = TerminalConfig { link_hover_delay_ms: 0, ..Default::default() };
...
"#;
    ...
    fn highlight_on_jump_defaults_true() {
        let cfg = TerminalConfig::default();
        assert!(cfg.highlight_on_jump, "highlight_on_jump must default to true");
    }
