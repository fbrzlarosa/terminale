...
pub fn default_known_hosts_path() -> PathBuf {
    directories::BaseDirs::new()
        .map_or_else(|| PathBuf::from(".ssh/known_hosts"), |b| b.home_dir().join(".ssh").join("known_hosts"))
}
...
pub fn default_openssh_config_path() -> PathBuf {
    directories::BaseDirs::new()
        .map_or_else(|| PathBuf::from(".ssh/config"), |b| b.home_dir().join(".ssh").join("config"))
}
...
mod tests {
    ...
    fn host_key_policy_roundtrips() {
        for policy in HostKeyPolicy::all() {
            ...
            let w = W { host_key_policy: policy };
    ...
    fn ssh_config_default_policy_is_accept_new() {
        assert_eq!(SshConfig::default().host_key_policy, HostKeyPolicy::AcceptNew);
    }

    #[test]
    fn ssh_config_default_validates() {
        SshConfig::default().validate().expect("default SshConfig must validate");
    }
...
"#;
    ...
    fn import_mode_roundtrips() {
        for mode in ImportOpenSshConfig::all() {
            ...
            let w = W { import_openssh_config: mode };
            let s = toml::to_string(&w).unwrap();
            let back: W = toml::from_str(&s).unwrap();
...
"#;
    ...
    fn parse_representative_config() {
        ...
        assert!(bastion.identity_file.is_some(), "bastion must have an identity_file");
    ...
    fn parse_wildcard_only_returns_empty() {
        let cfg = "Host *\n    ServerAliveInterval 60\n";
        let hosts = parse_ssh_config(cfg);
        assert!(hosts.is_empty(), "wildcard-only config must produce no hosts");
    }
...
"#;
        ...
        assert_eq!(cfg.openssh_config_path, PathBuf::from("/home/me/.ssh/config"));
        let s = toml::to_string(&cfg).unwrap();
        let back: SshConfig = toml::from_str(&s).unwrap();
