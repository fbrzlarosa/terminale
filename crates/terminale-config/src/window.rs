...
impl ZenHideElement {
    ...
    #[must_use]
    pub fn all() -> [Self; 4] {
        [Self::TabBar, Self::StatusBar, Self::PaneHeaders, Self::TitleBar]
    }
...
mod tests {
    ...
    fn padding_zero_is_valid() {
        let cfg = WindowConfig { padding: 0, ..WindowConfig::default() };
    ...
    fn padding_64_is_valid() {
        let cfg = WindowConfig { padding: 64, ..WindowConfig::default() };
        assert!(cfg.validate().is_ok());
    }
    ...
    fn padding_65_is_invalid() {
        let cfg = WindowConfig { padding: 65, ..WindowConfig::default() };
        assert!(
            cfg.validate().is_err(),
            "padding > 64 must fail validation"
        );
    }
    ...
    fn padding_range_validates() {
        let ok = WindowConfig { padding: 64, ..WindowConfig::default() };
        let err = WindowConfig { padding: 65, ..WindowConfig::default() };
