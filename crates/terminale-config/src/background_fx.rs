...
/// Unlike [`crate::keystroke_fx::KeystrokeFxConfig`] (transient particles at
...
mod tests {
    ...
    fn band_lifetime_bounds() {
        assert!(BackgroundFxConfig { band_lifetime_secs: 0.4, ..Default::default() }.validate().is_err());
        assert!(BackgroundFxConfig { band_lifetime_secs: 8.1, ..Default::default() }.validate().is_err());
        assert!(BackgroundFxConfig { band_lifetime_secs: 2.5, ..Default::default() }.validate().is_ok());
    }

    #[test]
    fn matrix_band_width_bounds() {
        assert!(BackgroundFxConfig { matrix_band_width: 0, ..Default::default() }.validate().is_err());
        assert!(BackgroundFxConfig { matrix_band_width: 9, ..Default::default() }.validate().is_err());
        assert!(BackgroundFxConfig { matrix_band_width: 3, ..Default::default() }.validate().is_ok());
    }

    #[test]
    fn max_emitters_bounds() {
        assert!(BackgroundFxConfig { max_emitters: 0, ..Default::default() }.validate().is_err());
        assert!(BackgroundFxConfig { max_emitters: 65, ..Default::default() }.validate().is_err());
        assert!(BackgroundFxConfig { max_emitters: 48, ..Default::default() }.validate().is_ok());
    }
}
