...
mod tests {
    ...
    fn rejects_size_zero() {
        let cfg = ClipboardHistoryConfig { size: 0, ..Default::default() };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn rejects_size_over_500() {
        let cfg = ClipboardHistoryConfig { size: 501, ..Default::default() };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn accepts_size_1_and_500() {
        let min = ClipboardHistoryConfig { size: 1, ..Default::default() };
        min.validate().expect("size 1 must validate");
        let max = ClipboardHistoryConfig { size: 500, ..Default::default() };
    ...
    fn roundtrip_toml() {
        ...
        let w = Wrap { clipboard_history: cfg };
